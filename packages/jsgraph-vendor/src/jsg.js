...
var JsgColor = {
  RGB: function (r, g, b) { return [r, g, b, 1]; }, RGBA: function (r, g, b, a) { return [r, g, b, a]; }, FromBitmap: function (c) { return [c[0] / 255, c[1] / 255, c[2] / 255, c[3] / 255]; }, BW: function (v) { return [v, v, v, 1]; }, Black: function () { return [0, 0, 0, 1]; }, White: function () { return [1, 1, 1, 1]; }, Ok: function (col) { return xArray(col); }, Alpha: function (col) { return xDefNum(col[3], 1); }, SetAlpha: function (col, a) { col[3] = xDefNum(a, 1); return col; }, SetRGBA: function (col, r, g, b, a) { col[0] = r; col[1] = g; col[2] = b; col[3] = a; return col; }, SetRGB: function (col, r, g, b) { col[0] = r; col[1] = g; col[2] = b; col[3] = 1; return col; }, SetBW: function (col, v) { col[0] = v; col[1] = v; col[2] = v; col[3] = 1; return col; }, SetBlack: function (col) { col[0] = 0; col[1] = 0; col[2] = 0; col[3] = 1; return col; }, SetWhite: function (col) { col[0] = 1; col[1] = 1; col[2] = 1; col[3] = 1; return col; }, Copy: function (src) { return [src[0], src[1], src[2], this.DefNum(src[3], 1)]; }, CopyTo: function (src, dest) { dest[0] = src[0]; dest[1] = src[1]; dest[2] = src[2]; dest[3] = this.DefNum(src[3], 1); return dest; }, Set: function (dest, src) { return this.CopyTo(src, dest); }, IsEqual: function (col1, col2) { return col1[0] == col2[0] && col1[1] == col2[1] && col1[2] == col2[2] && col1[3] == col2[3]; }, IsSameColor: function (col1, col2) { return col1[0] == col2[0] && col1[1] == col2[1] && col1[2] == col2[2]; }, Scale: function (col, s) { col[0] *= s; col[1] *= s; col[2] *= s; return col; }, Limit: function (col) { limit = this.Limit01; col[0] = limit(col[0]); col[1] = limit(col[1]); col[2] = limit(col[2]); col[3] = limit(col[3]); }, Add: function (col, add) { col[0] += add[0]; col[1] += add[1]; col[2] += add[2]; return col; }, Mult: function (col, mul) { col[0] *= mul[0]; col[1] *= mul[1]; col[2] *= mul[2]; return col; }, Blend: function (colRet, col1, col2, val) {
    var vali = 1 - val; for (var i = 0; i < 4; i++) { colRet[i] = vali * col1[i] + val * col2[i]; }
    return colRet;
  }, Combine: function (col1, col2) { var a2 = col2[3]; var r2 = a2 * col2[0]; var g2 = a2 * col2[1]; var b2 = a2 * col2[2]; var a1prime = 1 - col1[3]; col1[0] += r2 * a1prime; col1[1] += g2 * a1prime; col1[2] += b2 * a1prime; col1[3] += a2 * a1prime; return col1; }, ToString: function (col) {
    function normCol(cx) { cx = Math.round(cx * 255); return (cx > 255) ? 255 : ((cx < 0) ? 0 : cx); }
    function toHex(cx) { cx = normCol(cx); var hex = cx.toString(16); return cx < 16 ? "0" + hex : hex; }
    var a = normCol(this.DefNum(col[3], 1)); if (a == 255) { return '#' + toHex(col[0]) + toHex(col[1]) + toHex(col[2]); } else {
      return 'rgba('
    ...
    var Num = this.DefNum, Limit = this.Limit01; h = Limit(Num(h, 1)); s = Limit(Num(s, 1)); v = Limit(Num(v, 1)); a = Limit(Num(a, 1)); var r, g, b, hi; h *= 6; hi = Math.floor(h) % 6; h = h % 1; switch (hi) { case 0: r = 1; g = h; b = 0; break; case 1: r = 1 - h; g = 1; b = 0; break; case 2: r = 0; g = 1; b = h; break; case 3: r = 0; g = 1 - h; b = 1; break; case 4: r = h; g = 0; b = 1; break; default: r = 1; g = 0; b = 1 - h; }
    r = v * (1 - (1 - r) * s); g = v * (1 - (1 - g) * s); b = v * (1 - (1 - b) * s); return [r, g, b, a];
  }, HL: function (h, l, a) {
    l = this.Limit01(this.DefNum(l, 0.5)); var s, v; if (l < 0.5) { s = 1; v = 2 * l; } else { v = 1; s = 1 - 2 * (l - 0.5); }
    return this.HSV(h, s, v, a);
  }, DefNum: function (x, def) { return (typeof (x) === 'number') ? x : def; }, Limit01: function (x) { return (x < 0) ? 0 : ((x > 1) ? 1 : x); }
}; var JsgVect2 = {
  New: function (x, y) { return [x, y]; }, Copy: function (v) { return [v[0], v[1]]; }, CopyTo: function (v, to) { to[0] = v[0]; to[1] = v[1]; }, Set: function (v, x, y) {
    if (xArray(x)) { return this.Set(v, x[0], x[1]); }
    v[0] = x; v[1] = y; return v;
  }, Null: function () { return [0, 0]; }, IsNull: function (v) { return v[0] == 0 && v[1] == 0; }, Ok: function (v) { return xArray(v); }, Scale: function (v, s) { return [s * v[0], s * v[1]]; }, Add: function (v1, v2) { return [v1[0] + v2[0], v1[1] + v2[1]]; }, Sub: function (v1, v2) { return [v1[0] - v2[0], v1[1] - v2[1]]; }, Length: function (v) { var x = v[0], y = v[1]; return Math.sqrt(x * x + y * y); }, Length2: function (x, y) { return x * x + y * y; }, Norm: function (v) { var s = this.Length(v); if (s == 0) s = 1; return [v[0] / s, v[1] / s]; }, ScalarProd: function (v1, v2) { return v1[0] * v2[0] + v1[1] * v2[1]; }, VectProd: function (u, v) { return u[0] * v[1] - u[1] * v[0]; }, Rotate: function (v, ang) { var c = Math.cos(ang); var s = Math.sin(ang); return [c * v[0] - s * v[1], s * v[0] + c * v[1]]; }, AngleAbs: function (u, v, norm) {
    norm = xDefBool(norm, false); if (norm) { u = this.Norm(u); v = this.Norm(v); }
    var sp = this.ScalarProd(u, v); sp = (sp < -1) ? -1 : (sp > 1) ? 1 : sp; return Math.acos(sp);
  }, Angle: function (u, v, norm) {
    norm = xDefBool(norm, false); if (norm) { u = this.Norm(u); v = this.Norm(v); }
    var vp = this.VectProd(u, v); vp = (vp < -1) ? -1 : (vp > 1) ? 1 : vp; var sign = Math.asin(vp) < 0 ? -1 : 1; var sp = this.ScalarProd(u, v); sp = (sp < -1) ? -1 : (sp > 1) ? 1 : sp; return sign * Math.acos(sp);
  },
}; var JsgMat2 = {
  Zero: function () { return [[0, 0, 0], [0, 0, 0]]; }, Unit: function () { return [[1, 0, 0], [0, 1, 0]]; }, Ok: function (mat) { return xArray(mat); }, Copy: function (m) { return [[m[0][0], m[0][1], m[0][2]], [m[1][0], m[1][1], m[1][2]]]; }, CopyTo: function (src, dest) {
    for (var row = 0; row < 2; row++) { for (var col = 0; col < 3; col++) { dest[row][col] = src[row][col]; } }
    return dest;
  }, Set: function (dest, src) { return this.CopyTo(src, dest); }, RotatingToXY: function (x, y) {
    var vl = Math.sqrt(x * x + y * y); if (vl == 0) { x = 1; y = 0; } else { x /= vl; y /= vl; }
    return [[x, -y, 0], [y, x, 0]];
  }, Transformation: function (sx, sy, rot, x, y) { var cosRot = Math.cos(rot); var sinRot = Math.sin(rot); return [[cosRot * sx, -sinRot * sy, x], [sinRot * sx, cosRot * sy, y]]; }, Moving: function (x, y) { return [[1, 0, x], [0, 1, y]]; }, Scaling: function (sx, sy) { return [[sx, 0, 0], [0, sy, 0]]; }, Rotating: function (ang) { var c = Math.cos(ang); var s = Math.sin(ang); return [[c, -s, 0], [s, c, 0]]; }, Mult: function (matA, matB) {
    r = this.Null(); for (var i = 0; i < 2; i++) {
      for (var j = 0; j < 2; j++) { r[i][j] = matA[i][0] * matB[0][j] + matA[i][1] * matB[1][j]; }
      r[i][2] = matA[i][0] * matB[0][2] + matA[i][1] * matB[1][2] + matA[i][2];
    }
    return r;
  }, Move: function (mat, x, y) { return this.Mult(mat, this.Moving(x, y)); }, Rotate: function (mat, ang) { return this.Mult(mat, this.Rotating(ang)); }, Scale: function (mat, sx, sy) { return this.Mult(mat, this.Scaling(sx, sy)); }, Trans: function (mat, v) { var x = v[0] * mat[0][0] + v[1] * mat[0][1] + mat[0][2]; v[1] = v[0] * mat[1][0] + v[1] * mat[1][1] + mat[1][2]; v[0] = x; }, TransPolyXY: function (mat, polyX, polyY, size) { var l = xDefNum(size, polyX.length); for (var i = 0; i < l; i++) { var x = polyX[i] * mat[0][0] + polyY[i] * mat[0][1] + mat[0][2]; polyY[i] = polyX[i] * mat[1][0] + polyY[i] * mat[1][1] + mat[1][2]; polyX[i] = x; } },
}; function JsgRect(x, y, w, h) { this.Set(x, y, w, h); }
...
JsgPolygon.prototype.AddPoint = function (x, y, z) {
  this.X[this.Size] = x; this.Y[this.Size] = y; if (this.Z) this.Z[this.Size] = z; this.Size++
  return this;
}
JsgPolygon.prototype.AddPoint3D = function (p) {
  this.X[this.Size] = p[0]; this.Y[this.Size] = p[1]; this.Z[this.Size] = p[2]; this.Size++
  return this;
}
JsgPolygon.prototype.AddPoly = function (poly, offset) {
  offset = xDefNum(offset, 0); var xs = poly.X; var ys = poly.Y; var zs = poly.Z; var xd = this.X; var yd = this.Y; var zd = this.Z; var dSize = this.Size; var size = poly.Size; for (var i = offset; i < size; i++) { xd[dSize] = xs[i]; yd[dSize] = ys[i]; if (zd) zd[dSize] = zs[i]; dSize++; }
...
JsgPolygon.prototype.Close = function () {
  if (this.Size < 2) return false; if (this.IsSamePoint(0, this, this.Size - 1)) return false; if (this.Z) { this.AddPoint(this.X[0], this.Y[0], this.Z[0]); } else { this.AddPoint(this.X[0], this.Y[0]); }
  return true;
}
...
JsgPolygon.prototype.Copy = function (to, useNewArrays) {
  to = to || new JsgPolygon(this.Is3D()); if (useNewArrays) to.Init(this.Is3D); var toX = to.X
...
JsgPolygon.WorkArray = []; JsgPolygon.InvertArrays = function (xArr, yArr, zArr, size) {
  function InvertArray(a) { var last = Math.floor(size / 2) - 1; for (var i = 0, j = size - 1; i <= last; i++, j--) { var tmp = a[i]; a[i] = a[j]; a[j] = tmp; } }
...
JsgPolygon.RollArrays = function (xArr, yArr, zArr, n, size) {
  function RollArray(a) {
    var src = Math.abs(n) % size; if (n < 0) src = size - src; var newArr = JsgPolygon.WorkArray; for (var dest = 0; dest < size; dest++) { newArr[dest] = a[src++]; if (src >= size) src = 0; }
...
JsgPolygonList.prototype.NewPoly = function () {
  if (this.PolyList.length > this.Size) { this.PolyList[this.Size].Reset(); } else { this.PolyList[this.Size] = new JsgPolygon(this.Type3D); }
...
JsgPolygonList.prototype.AddPoly = function (polys, appendMode) {
  appendMode = xDefNum(appendMode, 0); if (!JsgPolygonList.Ok(polys)) return this.AddSinglePoly(polys, appendMode); var n = polys.Size; for (var i = 0; i < n; i++) { this.AddSinglePoly(polys.PolyList[i], appendMode); }
  return this;
}
JsgPolygonList.prototype.AddSinglePoly = function (poly, appendMode) {
  if (appendMode == 0) { this.NewPoly(); this.CurrPoly.AddPoly(poly, 0); } else {
    var offset = 0; if (this.Size == 0) { this.NewPoly(); } else { var currPoly = this.CurrPoly; if (poly.Size > 0 && currPoly.Size > 0 && poly.IsSamePoint(0, currPoly, currPoly.Size - 1)) { offset = 1; } else if (appendMode == 1) { this.NewPoly(); } }
...
JsgTrans2D.prototype.TransXY = function (x, y) {
  this.x = x; this.y = y; if (this.IsUnitTrans || !this.Enabled) return; if (this.IsMoveOnly) { this.x += this.a02; this.y += this.a12; return; }
...
JsgTrans2D.prototype.TransXY2 = function (x1, y1, x2, y2) {
  this.x1 = x1; this.y1 = y1; this.x2 = x2; this.y2 = y2; if (this.IsUnitTrans || !this.Enabled) return; if (this.IsMoveOnly) { this.x1 += this.a02; this.y1 += this.a12; this.x2 += this.a02; this.y2 += this.a12; return; }
...
function JsGraph(aParams) {
  ...
  if (this.EventHandlers.length > 0) {
    xOnDomReady(function CB_InstallEventHandlers() {
      var nHandlers = me.EventHandlers.length; for (var i = 0; i < nHandlers; i++) { var handler = me.EventHandlers[i]; me.AddEventHandler(handler.Event, handler.Func, handler.Capture); }
      me.EventHandlers = [];
    });
  }
...
JsGraph.prototype.IdCounter = 0; JsGraph.prototype.CreateCanvas = function (aParams) {
  JsGraph.prototype.IdCounter++; if (xStr(aParams.Id)) { this.Id = aParams.Id; } else { this.Id = 'JsGraph' + JsGraph.prototype.IdCounter; }
...
JsGraph.prototype.Draw = function () {
  ...
  if (this.BeforeResetFunc) {
    try { this.BeforeResetFunc(this); }
    catch (err) { }
  }
  if (this.AutoReset) {
    this.Reset(this.AutoClear); if (this.ClientResetFunc) {
      try { this.ClientResetFunc(this); }
      catch (err) { }
    }
  }
...
JsGraph.prototype.ParseHWInt = function (h, w) {
  var result; if (xDef(w) && this.IsNumericPercent(h)) { result = w * (parseFloat(h) / 100.0); } else if (xStr(h)) { result = parseFloat(h); } else { result = h; }
...
JsGraph.prototype.AdjustForHighResolutionDisplays = function () {
  var context = this.Context2D; var canvas = this.Canvas; if (this.HighResolution && this.DevicePixelRatio !== this.CanvasPixelRatio) {
    var ratio = this.PixelRatio; var oldWidth = this.CanvasWidth; var oldHeight = this.CanvasHeight; if (canvas.width != oldWidth * ratio) { canvas.width = oldWidth * ratio; canvas.height = oldHeight * ratio; }
    xStyle(canvas, 'width', oldWidth + 'px'); xStyle(canvas, 'height', oldHeight + 'px');
  } else {
    var ratio = 1; var width = this.CanvasWidth; var height = this.CanvasHeight; if (canvas.width != width) { canvas.width = width; canvas.height = height; }
...
JsGraph.prototype.UpdateCanvasSize = function (aContainerWidth) {
  if (!this.ContainerDiv) return; if (!xDef(aContainerWidth)) aContainerWidth = xWidth(this.ContainerDiv); this.UpdatePixelRatios(); if (this.CanvasRatioHW == 0) { this.CanvasWidth = aContainerWidth - 2 * this.BorderWidth; this.CanvasHeight = xHeight(this.ContainerDiv) - 2 * this.BorderWidth; if (!this.HighResSet || this.PixelRatio != this.LastPixelRatio) { this.AdjustForHighResolutionDisplays(); } } else {
    var width = aContainerWidth - 2 * this.BorderWidth; if (this.LastCanvasWidth == width && this.PixelRatio == this.LastPixelRatio) {
      if (!this.HighResSet) { this.AdjustForHighResolutionDisplays(); }
      return;
    }
...
JsGraph.prototype.SetAngleMeasure = function (am) {
  var old = this.AngleMeasure; if (am == 'rad') { this.AngleMeasure = 'rad'; } else { this.AngleMeasure = 'deg'; }
  return old;
}
...
JsGraph.prototype.MapWindow = function (aXcenter, aYcenter, aWidth, aHeight, aAlign) {
  aXcenter = xDefNum(aXcenter, 0); aYcenter = xDefNum(aYcenter, 0); aWidth = xDefNum(aWidth, 0); aHeight = xDefNum(aHeight, 0); aAlign = xDefNum(aAlign, 0); var vpAspectRatio = this.VpInnerWidth / this.VpInnerHeight; if (aWidth == 0) { aWidth = aHeight * vpAspectRatio; } else if (aHeight == 0) { if (vpAspectRatio != 0) aHeight = aWidth / vpAspectRatio; } else { var winAspectRatio = aWidth / aHeight; if (vpAspectRatio >= winAspectRatio) { var winWidth = aHeight * vpAspectRatio; var padding = (winWidth - aWidth) / 2; aXcenter -= aAlign * padding; aWidth = winWidth; } else { var winHeight = aWidth / vpAspectRatio; var padding = (winHeight - aHeight) / 2; aYcenter -= aAlign * padding; aHeight = winHeight; } }
...
JsGraph.prototype.SetClipRect = function (aX, aY, aWidth, aHeight, aTrans) {
  aX = xDefNum(aX, 0); aTrans = xDefStr(aTrans, ''); var oldTrans = this.Trans; if (aTrans != '') { this.SelectTrans(aTrans); }
...
JsGraph.prototype.SetScaleRef = function (aScaleRef, bLimitScalePix, bAutoScalePix, bScalePixInt) {
  if (xObj(aScaleRef)) { this.SetScaleRef(aScaleRef.ScaleRef, aScaleRef.LimitScalePix, aScaleRef.AutoScalePix, aScaleRef.ScalePixInt); return; }
...
JsGraph.prototype.ScalePix = function (aSize, bInt) {
  var m = aSize < 0 ? -1 : 1; var r = m * aSize * this.GetPixScaling(); if (bInt) { r = Math.round(r); if (r < 1) r = 1; }
  return m * r;
}
...
JsGraph.prototype.ScalePixMax = function (aSize, aMaxSize, bInt) {
  var m = aSize < 0 ? -1 : 1; var r = m * aSize * this.GetPixScaling(); if (r > aMaxSize) r = aMaxSize; if (bInt) { r = Math.round(r); if (r < 1) r = 1; }
  return m * r;
}
...
JsGraph.prototype.ScalePixMin = function (aSize, aMinSize, bInt) {
  var m = aSize < 0 ? -1 : 1; var r = m * aSize * this.GetPixScaling(); if (r < aMinSize) r = aMinSize; if (bInt) { r = Math.round(r); if (r < 1) r = 1; }
  return m * r;
}
...
JsGraph.prototype.ScalePixMinMax = function (aSize, aMinSize, aMaxSize, bInt) {
  var m = aSize < 0 ? -1 : 1; var r = m * aSize * this.GetPixScaling(); if (r < aMinSize) r = aMinSize; if (r > aMaxSize) r = aMaxSize; if (bInt) { r = Math.round(r); if (r < 1) r = 1; }
  return m * r;
}
...
JsGraph.prototype.WinToPixX = function (x, scalePix) { x = Math.abs(this.WinTrans.ScaleX) * x; scalePix = xDefBool(scalePix, this.AutoScalePix); if (scalePix) x = this.ScalePix(x); return x; }
JsGraph.prototype.WinToPixY = function (y, scalePix) { y = Math.abs(this.WinTrans.ScaleY) * y; scalePix = xDefBool(scalePix, this.AutoScalePix); if (scalePix) y = this.ScalePix(y); return y; }
JsGraph.prototype.PixToWinX = function (x, scalePix) { scalePix = xDefBool(scalePix, this.AutoScalePix); if (scalePix) x = this.ScalePix(x); x /= Math.abs(this.WinTrans.ScaleX); return x; }
JsGraph.prototype.PixToWinY = function (y, scalePix) { scalePix = xDefBool(scalePix, this.AutoScalePix); if (scalePix) y = this.ScalePix(y); y /= Math.abs(this.WinTrans.ScaleY); return y; }
...
JsGraph.prototype.BoxWHOverlapping = function (aBox1, aBox2) {
  if (!aBox1 || !aBox2) return false; var xmin1 = aBox1.x; var xmax1 = aBox1.x + aBox1.w; if (xmin1 > xmax1) { var tmp = xmin1; xmin1 = xmax1; xmax1 = tmp; }
...
JsGraph.prototype.MapToRange = function (val, range) {
  var absVal = Math.abs(val); var n = Math.floor(absVal / range); var newVal = absVal - n * range; if (val < 0) { newVal = range - newVal; if (newVal >= range) newVal -= range; } else { if (newVal < 0) newVal += range; }
  return newVal;
}
...
JsGraph.prototype.MakeUnityArcPolygon = function (aAngles) {
  var poly = this.WorkPoly.Reset(); var sin = Math.sin; var cos = Math.cos; if (aAngles.delta > 0) {
    var delta = aAngles.delta - this.MapToRange(aAngles.start, aAngles.delta); if (delta == 0) delta = aAngles.delta; var currAng = aAngles.start; var lastAng = aAngles.end - aAngles.delta / 1000; while (currAng < lastAng) { poly.AddPoint(cos(currAng), sin(currAng)); currAng += delta; delta = aAngles.delta; }
    poly.AddPoint(cos(aAngles.end), sin(aAngles.end));
  } else if (aAngles.delta < 0) {
    var delta = -this.MapToRange(aAngles.start, -aAngles.delta); if (delta == 0) delta = aAngles.delta; var currAng = aAngles.start; var lastAng = aAngles.end + aAngles.delta / 1000; while (currAng > lastAng) { poly.AddPoint(cos(currAng), sin(currAng)); currAng += delta; delta = aAngles.delta; }
    poly.AddPoint(cos(aAngles.end), sin(aAngles.end));
  }
  return poly;
}
...
JsGraph.prototype.SetTextAttrS = function (aFont, aSize, aColor, aAttr, aHPad, aVPad, aRot) { if (xAny(aFont)) this.SetTextFont(aFont); if (xAny(aSize)) this.SetTextSize(aSize); if (xAny(aRot)) this.SetTextRotation(aRot); if (xAny(aColor)) this.SetTextColor(aColor); if (xAny(aHPad)) this.SetTextPadding(aHPad, aVPad); aAttr = xDefStr(aAttr, ''); this.SetFontAttr(aAttr); this.SetTextAlignS(aAttr); }
...
JsGraph.prototype.SetTextRendering = function (aRenderMethod) {
  var oldRendering = this.TextRendering; if (!(this.Context2D.strokeText && this.Context2D.fillText)) aRenderMethod = 'html'; if (aRenderMethod == 'html') { this.TextRendering = 'html'; this.TextCanvasRendering = false; } else { this.TextRendering = 'canvas'; this.TextCanvasRendering = true; }
  return oldRendering;
}
...
JsGraph.prototype.SetFontAttr = function (aAttr) { var weight = aAttr.indexOf('b') >= 0 ? 'bold' : 'normal'; var style = aAttr.indexOf('i') >= 0 ? 'italic' : 'normal'; this.SetFontStyle(style, weight); }
...
JsGraph.prototype.SetTextAlignS = function (aAlign) { var h = 'center'; var v = 'middle'; if (aAlign.indexOf('N') >= 0) v = 'top'; if (aAlign.indexOf('S') >= 0) v = 'bottom'; if (aAlign.indexOf('W') >= 0) h = 'left'; if (aAlign.indexOf('E') >= 0) h = 'right'; this.SetTextHAlign(h); this.SetTextVAlign(v); }
...
JsGraph.prototype.SetTextPadding = function (aHPad, aVPad) {
  aHPad = xDefNum(aHPad, 0); aVPad = xDefNum(aVPad, aHPad); this.TextHPad = aHPad; this.TextVPad = aVPad; if (this.AutoScalePix) { aHPad = this.ScalePix(aHPad, this.ScalePixInt); aVPad = this.ScalePix(aVPad, this.ScalePixInt); }
...
JsGraph.prototype.Path = function (mode, clear) {
  mode = xDefNum(mode, 1); clear = xDefBool(clear, true); if (mode & 2) { if (this.DriverDrawPath(false, true)) { this.Context2D.fill(); } }
...
JsGraph.prototype.Clip = function (clear) {
  clear = xDefBool(clear, true); var oldClipEnabled = this.GraphClipEnabled; this.GraphClipEnabled = false; if (this.DriverDrawPath(false, false)) { this.Context2D.restore(); this.Context2D.save(); this.Context2D.clip(); }
...
JsGraph.prototype.NewCommonPathEle = function (t, x, y) {
  var ele, pool = this.CommonPathElePool; if (this.CommonPathElePoolSize < pool.length) { ele = pool[this.CommonPathElePoolSize++]; ele.t = t; ele.x = x; ele.y = y; } else { ele = { t: t, x: x, y: y }; pool[this.CommonPathElePoolSize++] = ele; }
  return ele;
}
JsGraph.prototype.NewArcPathEle = function (x, y, r, sa, ea, cc) {
  var ele, pool = this.ArcPathElePool; if (this.ArcPathElePoolSize < pool.length) { ele = pool[this.ArcPathElePoolSize++]; ele.t = 3; ele.x = x; ele.y = y; ele.r = r; ele.sa = sa; ele.ea = ea; ele.cc = cc; } else { ele = { t: 3, x: x, y: y, r: r, sa: sa, ea: ea, cc: cc }; pool[this.ArcPathElePoolSize++] = ele; }
  return ele;
}
JsGraph.prototype.NewBezierPathEle = function (cx1, cy1, cx2, cy2, ex, ey) {
  var ele, pool = this.BezierPathElePool; if (this.BezierPathElePoolSize < pool.length) { ele = pool[this.BezierPathElePoolSize++]; ele.t = 4; ele.cx1 = cx1; ele.cy1 = cy1; ele.cx2 = cx2; ele.cy2 = cy2; ele.ex = ex; ele.ey = ey; } else { ele = { t: 4, cx1: cx1, cy1: cy1, cx2: cx2, cy2: cy2, ex: ex, ey: ey }; pool[this.BezierPathElePoolSize++] = ele; }
  return ele;
}
...
JsGraph.prototype.PathAppendArc = function (x, y, r, sa, ea, cc, cont, close) {
  var arcStartX = x + r * Math.cos(sa); var arcStartY = y + r * Math.sin(sa); if (!cont) { this.PathMoveTo(arcStartX, arcStartY); }
...
JsGraph.prototype.PathAppendPolygon = function (xArray, yArray, cont, close, size) {
  var ctr = this.CurrTrans; var otr = this.GetObjTrans(); ctr.ObjTransXY(otr, xArray[0], yArray[0]); if (cont) { this.PathLineTo(ctr.x, ctr.y); } else { this.PathMoveTo(ctr.x, ctr.y); }
...
JsGraph.prototype.DriverPathPoly = new JsgPolygon(false, 'JsGraph.DriverPathPoly'); JsGraph.prototype.DriverPathClipPoly = new JsgPolygon(false, 'JsGraph.DrverPathClipPoly'); JsGraph.prototype.DriverPathClipPolyList = new JsgPolygonList(false, 'JsGraph.DriverPathClipPolyList'); JsGraph.prototype.DriverDrawPath = function (close, areaMode) {
  var plen = this.CurrPathSize; if (!this.GraphClipEnabled) { return this.DriverDrawPathPart(0, plen, true, close); }
  if (areaMode) {
    var quadrant = this.GetPathClipQuadrant(0, plen); if (quadrant == 0) { return this.DriverDrawPathPart(0, plen, true, close); } else if (quadrant == 1) { return false; }
    var poly = this.DriverGetPathPoly(0, 0, false); var polyClipped = this.ClipPolygonArea(poly, this.GraphClipInnerXmin, this.GraphClipInnerXmax, this.GraphClipInnerYmin, this.GraphClipInnerYmax, this.DriverPathClipPoly); return this.DriverDrawPathPoly(polyClipped, true, false);
  } else {
    var last; var from = 0; var newPath = true; while ((last = this.DriverNextPathEnd(from + 1)) > 0) {
      if (last - from > 1) { var closeLast = (last == plen && close); var quadrant = this.GetPathClipQuadrant(from, last); if (quadrant == 0) { if (this.DriverDrawPathPart(from, last, newPath, closeLast)) { newPath = false; } } else if (quadrant == 2) { var poly = this.DriverGetPathPoly(from, last, closeLast); var polyListClipped = this.ClipPolygon(poly, this.GraphClipInnerXmin, this.GraphClipInnerXmax, this.GraphClipInnerYmin, this.GraphClipInnerYmax, this.Context2D.lineWidth / 2, this.DriverPathClipPolyList); if (polyListClipped.Size > 0) { for (var i = 0; i < polyListClipped.Size; i++) { var polyClipped = polyListClipped.PolyList[i]; if (this.DriverDrawPathPoly(polyClipped, newPath, false)) { newPath = false; } } } } }
      from = last;
    }
    return !newPath;
  }
...
JsGraph.prototype.DriverGetPathPoly = function (from, to, closeLast) {
  var poly = this.DriverPathPoly.Reset(); var p = this.CurrPath; var plen = this.CurrPathSize; var closeArea = false; if (to == 0) { from = 0; to = plen; closeArea = true; }
  var lastMoveIx = from; for (var i = from; i < to; i++) {
    var c = p[i]; var t = c.t; if (t == 1) { poly.AddPoint(c.x, c.y); } else if (t == 2) {
      if (i > from) { if (p[i - 1].t == 2) { poly.RemoveLastPoint(); } else { var cl = p[lastMoveIx]; poly.AddPoint(cl.x, cl.y); } }
      poly.AddPoint(c.x, c.y); lastMoveIx = i;
    } else if (t == 0) { var c = p[lastMoveIx]; poly.AddPoint(c.x, c.y); } else if (t == 3) {
      var startAng = this.RadToAngle(c.sa); var endAng = this.RadToAngle(c.ea)
      var rad = c.cc ? -c.r : c.r; var ell = this.MakeEllipseArcPolygon(c.x, c.y, rad, c.r, 0, startAng, endAng); poly.AddPoly(ell);
    } else if (t == 4) { var cprev = p[i - 1]; var bezier = this.MakeBezierPolygon(cprev.x, cprev.y, c.cx1, c.cy1, c.cx2, c.cy2, c.ex, c.ey, this.NumBezierSegments); poly.AddPoly(bezier); }
  }
...
JsGraph.prototype.DriverDrawPathPart = function (from, to, newPath, close) {
  var p = this.CurrPath; var ctx = this.Context2D; if (newPath) ctx.beginPath(); for (var i = from; i < to; i++) { var c = p[i]; var t = c.t; if (t == 1) { ctx.lineTo(c.x, c.y); } else if (t == 2) { ctx.moveTo(c.x, c.y); } else if (t == 0) { ctx.closePath(); } else if (t == 3) { ctx.arc(c.x, c.y, c.r, c.sa, c.ea, c.cc); } else if (t == 4) { ctx.bezierCurveTo(c.cx1, c.cy1, c.cx2, c.cy2, c.ex, c.ey); } }
  if (close) ctx.closePath(); return to > from;
}
JsGraph.prototype.DriverDrawPathPoly = function (poly, newPath, close) {
  var size = poly.Size; if (size < 2) return false; var ctx = this.Context2D; var xs = poly.X; var ys = poly.Y; if (newPath) ctx.beginPath(); ctx.moveTo(xs[0], ys[0]); for (var i = 1; i < size; i++) { ctx.lineTo(xs[i], ys[i]); }
  if (close) ctx.closePath(); return true;
}
JsGraph.prototype.DriverNextPathEnd = function (from) {
  var plen = this.CurrPathSize; if (from >= plen) return -1; var p = this.CurrPath; for (var i = from; i < plen; i++) { var c = p[i]; var t = c.t; if (t == 2) { return i; } }
  return plen;
}
JsGraph.prototype.GetPathClipQuadrant = function (from, to) {
  function minmax(x, y) { if (x < xmin) xmin = x; if (x > xmax) xmax = x; if (y < ymin) ymin = y; if (y > ymax) ymax = y; }
...
JsGraph.prototype.DriverIsPathInsideRect = function (from, to, xmin, xmax, ymin, ymax) {
  var p = this.CurrPath; for (var i = from; i < to; i++) { var c = p[i]; var t = c.t; if (t == 1 || t == 2) { if (c.x < xmin || c.x > xmax || c.y < ymin || c.y > ymax) return false; } else if (t == 3) { var x = c.x - c.r; var y = c.y - c.r; if (x < xmin || x > xmax || y < ymin || y > ymax) return false; var x = c.x + c.r; var y = c.y + c.r; if (x < xmin || x > xmax || y < ymin || y > ymax) return false; } else if (t == 4) { if (c.cx1 < xmin || c.cx1 > xmax || c.cy1 < ymin || c.cy1 > ymax) return false; if (c.cx2 < xmin || c.cx2 > xmax || c.cy2 < ymin || c.cy2 > ymax) return false; if (c.ex < xmin || c.ex > xmax || c.ey < ymin || c.ey > ymax) return false; } }
  return true;
}
JsGraph.prototype.PenUp = function () { this.PenDown = false; }
JsGraph.prototype.MoveTo = function (x, y) {
  if (JsgVect2.Ok(x)) return this.MoveTo(x[0], x[1]); this.LastX = x; this.LastY = y; if (this.IsPathOpen) { var ctr = this.CurrTrans; ctr.ObjTransXY(this.GetObjTrans(), x, y); this.PathMoveTo(ctr.x, ctr.y); }
  this.PenDown = true; return this;
}
JsGraph.prototype.LineTo = function (x, y) {
  if (JsgVect2.Ok(x)) return this.LineTo(x[0], x[1]); var ctr = this.CurrTrans; if (this.IsPathOpen) { ctr.ObjTransXY(this.GetObjTrans(), x, y); if (this.PenDown) { this.PathLineTo(ctr.x, ctr.y); } else { this.PathMoveTo(ctr.x, ctr.y); } } else { if (this.PenDown) { this.WorkLineXArray[0] = this.LastX; this.WorkLineXArray[1] = x; this.WorkLineYArray[0] = this.LastY; this.WorkLineYArray[1] = y; this.DriverDrawPoly(this.WorkLineXArray, this.WorkLineYArray, 2, false, false); } }
  this.PenDown = true; this.LastX = x; this.LastY = y; return this;
}
JsGraph.prototype.WorkLineXArray = [0, 0]; JsGraph.prototype.WorkLineYArray = [0, 0]; JsGraph.prototype.Line = function (x1, y1, x2, y2, append) {
  if (JsgVect2.Ok(x1)) return this.Line(x1[0], x1[1], y1[0], y1[1], x2); append = xDefBool(append, false); if (this.IsPathOpen) {
    var ctr = this.CurrTrans; ctr.ObjTransXY2(this.GetObjTrans(), x1, y1, x2, y2); if (append) { this.PathLineTo(ctr.x1, ctr.y1); } else { this.PathMoveTo(ctr.x1, ctr.y1); }
    this.PathLineTo(ctr.x2, ctr.y2);
  } else { this.WorkLineXArray[0] = x1; this.WorkLineXArray[1] = x2; this.WorkLineYArray[0] = y1; this.WorkLineYArray[1] = y2; this.DriverDrawPoly(this.WorkLineXArray, this.WorkLineYArray, 2, false, false); }
...
JsGraph.prototype.Arrow = function (x1, y1, x2, y2, variant, mode, sym1, sym2) {
  if (JsgVect2.Ok(x1)) return this.Arrow(x1[0], x1[1], y1[0], y1[1], x2, y2); variant = xDefNum(variant, 1); mode = xDefNum(mode, 1 + 2); var ctr = this.CurrTrans; ctr.ObjTransXY2(this.GetObjTrans(), x1, y1, x2, y2); if (this.IsPathOpen && (mode & 8)) { this.PathLineTo(ctr.x1, ctr.y1); }
  if (x1 == x2 && y1 == y2) {
    if (this.IsPathOpen) { this.PathMoveTo(ctr.x2, ctr.y2); }
    this.PenDown = true; this.LastX = x2; this.LastY = y2; return this;
  }
  var otr = this.ObjTrans; var cnvsX1 = ctr.x1, cnvsY1 = ctr.y1, cnvsX2 = ctr.x2, cnvsY2 = ctr.y2; otr.TransXY2(x1, y1, x2, y2); var x1orig = otr.x1, y1orig = otr.y1, x2orig = otr.x2, y2orig = otr.y2; var x1corr = otr.x1, y1corr = otr.y1, x2corr = otr.x2, y2corr = otr.y2; var oldTransState = otr.Enable(false); if ((variant & 8) && (variant & 1)) {
    var v = JsgVect2.New(cnvsX2 - cnvsX1, cnvsY2 - cnvsY1); var vd = JsgVect2.Scale(JsgVect2.Norm(v), -this.Context2D.lineWidth / 2); var vs = JsgVect2.Add(v, vd); if (JsgVect2.ScalarProd(vs, v) <= 0) { variant |= 4; }
    x2corr = ctr.InvTransX(vs[0] + cnvsX1); y2corr = ctr.InvTransY(vs[1] + cnvsY1);
  }
  if ((variant & 8) && (variant & 2)) {
    var v = JsgVect2.New(cnvsX1 - cnvsX2, cnvsY1 - cnvsY2); var vd = JsgVect2.Scale(JsgVect2.Norm(v), -this.Context2D.lineWidth / 2); var vs = JsgVect2.Add(v, vd); if (JsgVect2.ScalarProd(vs, v) <= 0) { variant |= 4; }
    var x1corr = ctr.InvTransX(vs[0] + cnvsX2); var y1corr = ctr.InvTransY(vs[1] + cnvsY2);
  }
  if (!(variant & 4)) {
    var drawit = true; if (variant & 8) { var v1 = JsgVect2.New(x2orig - x1orig, y2orig - y1orig); var v2 = JsgVect2.New(x2corr - x1corr, y2corr - y1corr); drawit = (JsgVect2.ScalarProd(v1, v2) > 0); }
...
JsGraph.prototype.PolygonArrow = function (xArray, yArray, variant, lineMode, arrowMode, size, sym1, sym2) {
  if (JsgPolygon.Ok(xArray)) { return this.PolygonArrow(xArray.X, xArray.Y, yArray, variant, lineMode, xArray.Size, arrowMode, size); }
  variant = xDefNum(variant, 1); lineMode = xDefNum(lineMode, 1); arrowMode = xDefNum(arrowMode, 1 + 2); size = xDefNum(size, xArray.length); if (size < 1) return this; if (size == 1) return this.Line(xArray[0], yArray[0], xArray[0], yArray[0], ((lineMode & 8) > 0)); if (size == 2) return this.Arrow(xArray[0], yArray[0], xArray[1], yArray[1], variant, arrowMode); var last = size - 1; if (this.IsPathOpen && (lineMode & 8)) { var ctr = this.CurrTrans; ctr.ObjTransXY(this.GetObjTrans(), xArray[0], yArray[0]); this.PathLineTo(ctr.x, ctr.y); }
  if (!(variant & 4)) {
    if (lineMode & 4) { this.Polygon(xArray, yArray, lineMode & ~8, size); } else {
      var skip = ((variant & 1) && (variant & 2) && (size == 3)); if (!skip) {
        if (this.IsPathOpen) { this.Polygon(xArray, yArray, lineMode & 3, size); } else {
          var x1 = xArray[0]; var y1 = yArray[0]; var x2 = xArray[last]; var y2 = yArray[last]; if (variant & 2) { xArray[0] = xArray[1]; yArray[0] = yArray[1]; }
          if (variant & 1) {
  ...
  var hideSeg = this.IsPathOpen ? 4 : 0; if (variant & 2) {
    var x1 = xArray[0], y1 = yArray[0]; var x2 = xArray[1], y2 = yArray[1]; var i = 1; while ((i < last) && (x1 == x2) && (y1 == y2)) { i++; x2 = xArray[i]; y2 = yArray[i]; }
    this.Arrow(x1, y1, x2, y2, (variant & 8) + hideSeg + 2, arrowMode & (1 + 2), sym1);
  }
  if (variant & 1) {
    var x1 = xArray[last - 1], y1 = yArray[last - 1]; var x2 = xArray[last], y2 = yArray[last]; var i = last - 1; while ((i > 0) && (x1 == x2) && (y1 == y2)) { i--; x1 = xArray[i]; y1 = yArray[i]; }
...
JsGraph.prototype.Rect = function (x1, y1, x2, y2, mode, roll) {
  if (!xDef(x1)) {
    var oldTransState = this.ObjTrans.Enable(false); this.Rect(this.GetFrame(), 1)
...
JsGraph.prototype.DriverDrawRect = function (x1, y1, x2, y2, mode, roll) {
  mode = xDefNum(mode, 1); roll = xDefNum(roll, 0); var ctr = this.CurrTrans; var otr = this.ObjTrans; var inv = !!(mode & 4); if (this.IsPathOpen) { var poly = this.MakeRectPolygon(x1, y1, x2, y2, inv, roll); this.Polygon(poly, mode & 11); return; }
  ...
  if (quadrant == 0) {
    if (mode & 2) { ctx.fillRect(ctr.x1, ctr.y1, ctr.x2 - ctr.x1, ctr.y2 - ctr.y1); }
    if (mode & 1) {
      var oldJoin = ctx.lineJoin; var oldCap = ctx.lineCap; if (oldJoin == 'round') { ctx.lineCap = 'round'; } else { ctx.lineJoin = 'miter'; ctx.lineCap = 'square'; }
      ctx.strokeRect(ctr.x1, ctr.y1, ctr.x2 - ctr.x1, ctr.y2 - ctr.y1); ctx.lineJoin = oldJoin; ctx.lineCap = oldCap;
    }
  } else if (quadrant == 2) { this.RectAsPolygon(x1, y1, x2, y2, mode, inv, roll); }
}
JsGraph.prototype.RectAsPolygon = function (x1, y1, x2, y2, mode, inv, roll) {
  var poly = this.MakeRectPolygon(x1, y1, x2, y2, inv, roll); var oldJoin = this.LineJoin; var oldCap = this.LineCap; if (oldJoin == 'round') { this.SetLineCap('round'); } else { this.SetLineJoin('miter'); this.SetLineCap('square'); }
...
JsGraph.prototype.MakeRectPolygon = function (x1, y1, x2, y2, inverse, roll) {
  if (JsgVect2.Ok(x1)) return this.MakeRectPolygon(x1[0], x1[1], y1[0], y1[1], x2, y2); if (xObj(x1)) { if (JsgRect.Ok(x1)) return this.MakeRectPolygon(x1.x, x1.y, x1.x + x1.w, x1.y + x1.h, y1, x2); return this.MakeRectPolygon(x1.xmin, x1.ymin, x1.xmax, x1.ymax, y1, x2); }
...
JsGraph.prototype.MakeArcFromPoints = function (x1, y1, x2, y2, r, big) {
  var arc = { x: x1, y: y1, r: r, start: 0, end: 0 }; var absr = Math.abs(r); var mx = (x2 - x1) / 2; var my = (y2 - y1) / 2; var ml = Math.sqrt(mx * mx + my * my); if (ml == 0) return arc; var hl = 0; if (absr > ml) hl = Math.sqrt(absr * absr - ml * ml); var hx = -hl * my / ml; var hy = hl * mx / ml; if ((r < 0) ^ big) { hx = -hx; hy = -hy; }
...
JsGraph.prototype.MakeEllipseArcPolygon = function (x, y, rx, ry, rot, start, end, rPixel, nSegments) {
  if (JsgVect2.Ok(x)) return this.MakeEllipseArcPolygon(x[0], x[1], y, rx, ry, rot, start, end, rPixel); ry = xDefNum(ry, Math.abs(rx)); rot = xDefNum(rot, 0); start = xDefNum(start, 0); end = xDefNum(end, start + this.RadToAngle(2 * Math.PI)); var abs = Math.abs, max = Math.max; var ctr = this.CurrTrans; var otr = this.ObjTrans; var absRx = abs(rx); var absRy = abs(ry); if (!xDef(rPixel) && !xDef(nSegments)) { var maxR = max(absRx, absRy); var s = otr.MaxScaling(); var cnvsRx = abs(s * ctr.ScaleX * maxR); var cnvsRy = abs(s * ctr.ScaleY * maxR); rPixel = max(cnvsRx, cnvsRy); }
  rot = this.AngleToRad(rot); start = this.AngleToRad(start); end = this.AngleToRad(end); var delta; if (xDef(nSegments)) { delta = Math.abs(start - end) / nSegments; } else { delta = this.CompDeltaAngle(rPixel, this.CurvePrecision / this.DevicePixelRatio); }
  var angles = { delta: delta, start: start, end: end }; if (rx < 0) angles.delta *= -1; this.NormalizeAngles(angles); rot = this.NormalizeAngle(rot); var poly = this.MakeUnityArcPolygon(angles); var mat = JsgMat2.Transformation(absRx, absRy, rot, x, y); JsgMat2.TransPolyXY(mat, poly.X, poly.Y, poly.Size); return poly;
}
JsGraph.prototype.Circle = function (x, y, r, mode, startAngle) {
  if (JsgVect2.Ok(x)) return this.Circle(x[0], x[1], y, r, mode); startAngle = xDefNum(startAngle, 0); var start = startAngle; var end = startAngle + this.RadToAngle(2 * Math.PI); if (r < 0) { start = end; end = startAngle; }
  this.Arc(x, y, r, start, end, mode); return this;
}
JsGraph.prototype.Arc = function (x, y, r, start, end, mode) {
  if (JsgVect2.Ok(x)) return this.Arc(x[0], x[1], y, r, start, end); var ctr = this.CurrTrans; var cnvsRX = Math.abs(ctr.ScaleX * r); var cnvsRY = Math.abs(ctr.ScaleY * r); var cnvsRDiff = Math.abs(cnvsRX - cnvsRY); if (this.DisableNativeArc || !this.ObjTrans.IsMoveOnly || cnvsRDiff > this.CurvePrecision / this.DevicePixelRatio) { this.EllipseArcAsPolygon(x, y, r, Math.abs(r), 0, start, end, mode); } else { this.DriverDrawArc(x, y, r, start, end, mode); }
  return this;
}
...
JsGraph.prototype.DriverDrawArc = function (x, y, r, start, end, mode) {
  start = xDefNum(start, 0); end = xDefNum(end, start + this.RadToAngle(2 * Math.PI)); mode = xDefNum(mode, 1); var cnvsStart = this.AngleToRad(start); var cnvsEnd = this.AngleToRad(end); var ctx = this.Context2D; var ctr = this.CurrTrans; ctr.ObjTransXY(this.GetObjTrans(), x, y); var cnvsRX = Math.abs(ctr.ScaleX * r); var angles = { delta: r, start: cnvsStart, end: cnvsEnd }; if (ctr.ScaleX * ctr.ScaleY < 0) { angles.delta *= -1; angles.start *= -1; angles.end *= -1; }
  this.NormalizeAngles(angles); var inverse = (angles.delta < 0); if (this.IsPathOpen) { this.PathAppendArc(ctr.x, ctr.y, cnvsRX, angles.start, angles.end, inverse, ((mode & 8) > 0), ((mode & 4) > 0)); } else {
    var quadrant = 0; if (this.GraphClipEnabled) { quadrant = this.GetCircleClipQuadrant(ctr.x, ctr.y, cnvsRX); }
    if (quadrant == 0) {
      if (mode & 2) { ctx.beginPath(); ctx.arc(ctr.x, ctr.y, cnvsRX, angles.start, angles.end, inverse); ctx.fill(); }
      if (mode & 1) { ctx.beginPath(); ctx.arc(ctr.x, ctr.y, cnvsRX, angles.start, angles.end, inverse); if (mode & 4) ctx.closePath(); ctx.stroke(); }
    } else if (quadrant == 2) { this.EllipseArcAsPolygon(x, y, r, Math.abs(r), 0, start, end, mode); }
  }
...
JsGraph.prototype.Ellipse = function (x, y, rx, ry, rot, mode, startAngle) {
  if (JsgVect2.Ok(x)) return this.Ellipse(x[0], x[1], y, rx, ry, rot, mode); startAngle = xDefNum(startAngle, 0); var start = startAngle; var end = startAngle + this.RadToAngle(2 * Math.PI); if (rx < 0) { start = end; end = startAngle; }
...
JsGraph.prototype.IsClosedPolygon = function (xArray, yArray, size) {
  if (JsgPolygon.Ok(xArray)) return this.IsClosedPolygon(xArray.X, xArray.Y, xArray.Size); size = xDefNum(size, xArray.length); var closed = false; var last = size - 1; if (last >= 2) { var refLen = 0.5 / this.DevicePixelRatio; if (JsgVect2.Length2(xArray[0] - xArray[last], yArray[0] - yArray[last]) <= (refLen * refLen)) closed = true; }
  return closed;
}
JsGraph.prototype.EllipseArcAsPolygon = function (x, y, rx, ry, rot, start, end, mode) {
  mode = xDefNum(mode, 1); var ell = this.MakeEllipseArcPolygon(x, y, rx, ry, rot, start, end); var closed = this.IsClosedPolygon(ell.X, ell.Y, ell.Size); if (closed) {
    var ctx = this.Context2D
    var oldJoin = ctx.lineJoin; var oldCap = ctx.lineCap; ctx.lineJoin = 'round'; ctx.lineCap = 'round'; this.Polygon(ell, mode); ctx.lineJoin = oldJoin; ctx.lineCap = oldCap;
  } else { this.Polygon(ell, mode); }
}
JsGraph.prototype.DriverWorkPoly = new JsgPolygon(false, 'JsGraph.DriverWorkPoly'); JsGraph.prototype.DriverWorkClipPoly = new JsgPolygon(false, 'JsGraph.DriverWorkClipPoly'); JsGraph.prototype.DriverWorkClipPolyList = new JsgPolygonList(false, 'JsGraph.DriverWorkClipPolyList'); JsGraph.prototype.DriverDrawPoly = function (xArray, yArray, size, fillMode, close, poly) {
  var ctr = this.CurrTrans; var otr = this.GetObjTrans(); var ctx = this.Context2D; if (!xDef(poly)) { poly = this.DriverWorkPoly.Reset(); poly.Quadrant = -1; for (var i = 0; i < size; i++) { ctr.ObjTransXY(otr, xArray[i], yArray[i]); poly.AddPoint(ctr.x, ctr.y); } }
  if (this.GraphClipEnabled) {
    var quadrant = poly.Quadrant; if (quadrant == -1) { quadrant = this.GetPolygonClipQuadrant(poly.X, poly.Y, poly.Size); poly.Quadrant = quadrant; }
    if (fillMode) {
      var polyClipped = null; if (quadrant == 0) { polyClipped = poly; } else if (quadrant == 2) { var didClosePoly = this.ClosePolygon(poly); polyClipped = this.ClipPolygonArea(poly, this.GraphClipInnerXmin, this.GraphClipInnerXmax, this.GraphClipInnerYmin, this.GraphClipInnerYmax, this.DriverWorkClipPoly); if (didClosePoly) poly.RemoveLastPoint(); }
      if (polyClipped) { if (this.DriverDrawPathPoly(polyClipped, true, false)) { ctx.fill(); } }
    } else {
      if (quadrant == 0) { if (this.DriverDrawPathPoly(poly, true, close)) { ctx.stroke(); } } else if (quadrant == 2) {
        var didClosePoly = false; if (close) didClosePoly = this.ClosePolygon(poly); var polyListClipped = this.ClipPolygon(poly, this.GraphClipInnerXmin, this.GraphClipInnerXmax, this.GraphClipInnerYmin, this.GraphClipInnerYmax, ctx.lineWidth / 2, this.DriverWorkClipPolyList); if (polyListClipped.Size > 0) {
          var newPath = true; for (var i = 0; i < polyListClipped.Size; i++) { var polyClipped = polyListClipped.PolyList[i]; if (this.DriverDrawPathPoly(polyClipped, newPath, false)) { newPath = false; } }
          if (!newPath) ctx.stroke();
        }
        if (didClosePoly) poly.RemoveLastPoint();
      }
    }
  } else { if (this.DriverDrawPathPoly(poly, true, close)) { if (fillMode) { ctx.fill(); } else { ctx.stroke(); } } }
  return poly;
}
JsGraph.prototype.DriverDrawBezierCurve = function (sx, sy, cx1, cy1, cx2, cy2, ex, ey, mode, nSegments) {
  mode = xDefNum(mode, 1); var ctx = this.Context2D; var ctr = this.CurrTrans; var otr = this.GetObjTrans(); ctr.ObjTransXY(otr, sx, sy); var cnvsSx = ctr.x, cnvsSy = ctr.y; ctr.ObjTransXY(otr, cx1, cy1); var cnvsCx1 = ctr.x, cnvsCy1 = ctr.y; ctr.ObjTransXY(otr, cx2, cy2); var cnvsCx2 = ctr.x, cnvsCy2 = ctr.y; ctr.ObjTransXY(otr, ex, ey); var cnvsEx = ctr.x, cnvsEy = ctr.y; if (this.IsPathOpen) {
    if (mode & 8) { this.PathLineTo(cnvsSx, cnvsSy); } else { this.PathMoveTo(cnvsSx, cnvsSy); }
    this.PathAppendBezierTo(cnvsCx1, cnvsCy1, cnvsCx2, cnvsCy2, cnvsEx, cnvsEy); if (mode & 4) { this.PathLineTo(cnvsSx, cnvsSy); }
  } else {
    if (mode & 8) { if (this.LastX != sx || this.LastY != sy) { this.Line(this.LastX, this.LastY, sx, sy); } }
    ...
    if (quadrant == 0) {
      if (mode & 2) { ctx.beginPath(); ctx.moveTo(cnvsSx, cnvsSy); ctx.bezierCurveTo(cnvsCx1, cnvsCy1, cnvsCx2, cnvsCy2, cnvsEx, cnvsEy); ctx.fill(); }
      if (mode & 1) { ctx.beginPath(); ctx.moveTo(cnvsSx, cnvsSy); ctx.bezierCurveTo(cnvsCx1, cnvsCy1, cnvsCx2, cnvsCy2, cnvsEx, cnvsEy); if (mode & 4) ctx.closePath(); ctx.stroke(); }
    } else if (quadrant == 2) { this.BezierCurveAsPolygon(sx, sy, cx1, cy1, cx2, cy2, ex, ey, mode, nSegments); }
  }
...
JsGraph.prototype.Polygon = function (xArray, yArray, mode, size) {
  if (JsgPolygon.Ok(xArray)) return this.Polygon(xArray.X, xArray.Y, yArray, xArray.Size); mode = xDefNum(mode, 1); size = xDefNum(size, xArray.length); if (size < 1) return this; if (this.IsPathOpen) { this.PathAppendPolygon(xArray, yArray, ((mode & 8) > 0), ((mode & 4) > 0), size); } else {
    var poly; if (mode & 2) { poly = this.DriverDrawPoly(xArray, yArray, size, true, false); }
    if (mode & 1) { this.DriverDrawPoly(xArray, yArray, size, false, (mode & 4) > 0, poly); }
  }
  var i = (mode & 4) ? 0 : size - 1; this.PenDown = true; this.LastX = xArray[i]; this.LastY = yArray[i]; return this;
}
JsGraph.prototype.PolygonList = function (polyList, mode) {
  if (!JsgPolygonList.Ok(polyList)) return g.Polygon(polyList, mode); for (var i = 0; i < polyList.Size; i++) { this.Polygon(polyList.PolyList[i], mode); }
  return this;
}
JsGraph.prototype.ClosePolygon = function (poly) {
  var last = poly.Size - 1; if (last < 1) return false; if (poly.X[0] != poly.X[last] || poly.Y[0] != poly.Y[last]) { poly.AddPoint(poly.X[0], poly.Y[0]); return true; }
  return false;
}
...
JsGraph.prototype.WorkPolyListClipped = new JsgPolygonList(false, 'JsGraph.WorkPolyListClipped'); JsGraph.prototype.ClipPolygon = function (poly, xmin, xmax, ymin, ymax, extend, polyListClipped) {
  extend = xDefNum(extend, 0); if (extend != 0) { xmin -= extend; xmax += extend; ymin -= extend; ymax += extend; }
...
JsGraph.prototype.ClipPolygonAtLine = function (poly, clipCoord, clipAtMax, clipHorizontal, polyClipped) {
  function AddPointToPolyClipped(x, y) { if (clipHorizontal) { polyClipped.AddPoint(y, x); } else { polyClipped.AddPoint(x, y); } }
  ...
  isP1Inside = IsInside(polyX[0]); if (poly.Size == 1) {
    if (isP1Inside) { if (isBorderClipMode) polyClipped.NewPoly(); polyClipped.AddPoint(poly.X[0], poly.Y[0]); }
    return;
  }
  var polyClosed = false; if (!isBorderClipMode) { polyClosed = this.ClosePolygon(poly); }
  var isLastP2Added = false; var nlast = poly.Size - 2; for (var i = 0; i <= nlast; i++) {
    var i2 = i + 1; isP2Inside = IsInside(polyX[i2]); if (isP1Inside && isP2Inside) {
      if (!isLastP2Added) { if (isBorderClipMode) polyClipped.NewPoly(); AddPointToPolyClipped(polyX[i], polyY[i]); }
      AddPointToPolyClipped(polyX[i2], polyY[i2]); isLastP2Added = true;
    } else if (isP1Inside != isP2Inside) {
      var intersectCoord = this.ClipIntersectionCoord(polyX[i], polyY[i], polyX[i2], polyY[i2], clipCoord); if (isP1Inside) {
        if (!isLastP2Added) { if (isBorderClipMode) polyClipped.NewPoly(); AddPointToPolyClipped(polyX[i], polyY[i]); }
        AddPointToPolyClipped(clipCoord, intersectCoord); isLastP2Added = false;
      } else { if (isBorderClipMode) polyClipped.NewPoly(); AddPointToPolyClipped(clipCoord, intersectCoord); AddPointToPolyClipped(polyX[i2], polyY[i2]); isLastP2Added = true; }
    }
...
JsGraph.prototype.GetPolygonClipQuadrant = function (xArray, yArray, size) {
  if (this.IsPolygonArrayInsideRect(xArray, yArray, size, this.GraphClipInnerXmin, this.GraphClipInnerXmax, this.GraphClipInnerYmin, this.GraphClipInnerYmax)) { return 0; }
...
JsGraph.prototype.IsPolygonArrayInsideRect = function (xArray, yArray, size, xmin, xmax, ymin, ymax) {
  for (var i = 0; i < size; i++) { if (xArray[i] < xmin || xArray[i] > xmax || yArray[i] < ymin || yArray[i] > ymax) return false; }
  return true;
}
...
JsGraph.prototype.BezierCurve = function (sx, sy, cx1, cy1, cx2, cy2, ex, ey, mode, nSegments) {
  if (JsgVect2.Ok(sx)) { return this.BezierCurve(sx[0], sx[1], sy[0], sy[1], cx1[0], cx1[1], cy1[0], cy1[1], cx2, cx1); }
...
JsGraph.prototype.MakeBezierPolygon = function (sx, sy, cx1, cy1, cx2, cy2, ex, ey, nSegments, add, polyRet) {
  if (JsgVect2.Ok(sx)) { return this.MakeBezierPolygon(sx[0], sx[1], sy[0], sy[1], cx1[0], cx1[1], cy1[0], cy1[1], cx2, cy2, ex); }
  if (JsgPolygon.Ok(sx)) {
    polyRet = polyRet || this.WorkPoly2; var startIx = xDefNum(sy, 0); if (sx.Size < startIx + 4) { add = xDefBool(add, false); if (!add) polyRet.Reset(); return polyRet; }
...
JsGraph.prototype.MakeSplineCurve = function (xArray, yArray, tension, mode, size, nSegments, polyRet) {
  if (JsgPolygon.Ok(xArray)) { return this.MakeSplineCurve(xArray.X, xArray.Y, yArray, tension, xArray.Size, mode); }
  var bezierPoly = this.SplineCurve(xArray, yArray, tension, mode, size); polyRet = polyRet || this.WorkPoly2; polyRet.Reset(); var first = 0; var last = bezierPoly.Size - 1; if (!(mode & 4) && (mode & 64)) { if ((mode & 16) && (last - first > 3)) first += 3; if ((mode & 32) && (last - first > 3)) last -= 3; }
...
JsGraph.prototype.ComputeBezierControlPoints = function (poly, tension, last, symmetric) {
  function LengthFor(side1, side2) { return Math.sqrt(side1 * side1 + side2 * side2); }
  var fa, fb; var px = poly.X; var py = poly.Y; for (var i = 1; i <= last; i++) {
    var pivot = 3 * i; var left = pivot - 3; var right = pivot + 3; var ca = pivot - 1; var cb = pivot + 1; var d01 = LengthFor(px[pivot] - px[left], py[pivot] - py[left]); var d12 = LengthFor(px[right] - px[pivot], py[right] - py[pivot]); var d = d01 + d12; if (d > 0) { fa = tension * d01 / d; fb = tension * d12 / d; if (symmetric) { if (fa > fb) { fa = fb; } else if (fb > fa) { fb = fa; } } } else { fa = 0; fb = 0; }
...
JsGraph.prototype.SplineCurve = function (xArray, yArray, tension, mode, size) {
  if (JsgPolygon.Ok(xArray)) { return this.SplineCurve(xArray.X, xArray.Y, yArray, tension, xArray.Size); }
  tension = xDefNum(tension, 0.5); mode = xDefNum(mode, 1); size = xDefNum(size, xArray.length); if (size < 2) return this; if (size == 2) { return this.Line(xArray[0], yArray[0], xArray[1], yArray[1], (mode & 8) > 0); }
  ...
  var last = (mode & 4) ? size : size - 2; if (firstIsControlPoint) last--; if (lastIsControlPoint) last--; this.ComputeBezierControlPoints(poly, tension, last, (mode & 128) ? true : false); if (mode & 4) { var i = poly.Size - 3; poly.X[1] = poly.X[i]; poly.Y[1] = poly.Y[i]; poly.Size = finalPolySize; }
  ...
  if (!oldIsPathOpen) {
    if (closedBorder) {
      var ctx = this.Context2D
      var oldCap = ctx.lineCap; ctx.lineCap = 'round'; this.Path(mode & 3); ctx.lineCap = oldCap;
    } else { this.Path(mode & 3); }
  }
  return this.WorkPoly;
}
JsGraph.prototype.DrawImage = function (img, x1, y1, w1, h1, x2, y2, w2, h2, fit, callback) {
  var x, y, w, h, sx, sy, sw, sh
  sx = 0; sy = 0; sw = 0; sh = 0; var effFit = ''; if (xObj(x1)) { if (xObj(y1)) { sx = x1.x; sy = x1.y; sw = x1.w; sh = x1.h; x = x2.x; y = x2.y; w = xDefNum(x2.w, 0); h = xDefNum(x2.h, 0); if (xStr(w1)) { effFit = w1; callback = h1; } else { callback = w1; } } else if (xStr(y1)) { sx = x1.x; sy = x1.y; sw = x1.w; sh = x1.h; effFit = y1; callback = w1; } else { x = x1.x; y = x1.y; w = xDefNum(x1.w, 0); h = xDefNum(x1.h, 0); if (xStr(y1)) { effFit = y1; callback = w1; } else { callback = y1; } } } else if (xStr(x1)) { effFit = x1; callback = y1; } else if (xNum(x1)) { if (xNum(x2)) { sx = x1; sy = y1; sw = w1; sh = h1; x = x2; y = y2; if (xNum(w2)) { w = w2; h = h2; if (xStr(fit)) { effFit = fit; } else { callback = fit; } } else if (xStr(w2)) { w = 0; h = 0; effFit = w2; callback = h2; } else { w = 0; h = 0; callback = w2; } } else if (xStr(x2)) { sx = x1; sy = y1; sw = w1; sh = h1; effFit = x2; callback = y2; } else { x = x1; y = y1; w = xDefNum(w1, 0); h = xDefNum(h1, 0); if (xNum(w1)) { callback = x2; } else { callback = w1; } } } else { x = 0; y = 0; w = 0; h = 0; callback = x1; }
  if (xStr(img) || xNum(img)) {
    var imgID; if (xNum(img)) { imgID = img; } else { var imgID = IC.FindImage(img); }
    if (imgID == -1) { var me = this; imgID = IC.LoadImage(img, function (imgID) { if (callback) callback(me, imgID); if (me.AutoRedrawOnImgLoad) me.Redraw(); }); }
    if (!IC.IsLoaded(imgID)) return false; img = IC.Image(imgID);
  }
  if (effFit != '') {
    var scaleW = 1; var scaleH = 1; var moveX = 0; var moveY = 0; if (effFit.indexOf('scale') != -1) { scaleW = w; scaleH = h; moveX = x; moveY = y; }
    x = 0; y = 0; w = this.CanvasWidth; h = this.CanvasHeight; if (effFit.indexOf('viewport') != -1) { x = this.VpXmin; y = this.VpYmin; w = this.VpInnerWidth; h = this.VpInnerHeight; }
    if (effFit.indexOf('-in') != -1) { var iw = img.naturalWidth; if (iw == 0) iw = w; var ih = img.naturalHeight; if (ih == 0) ih = h; var ir = iw / ih; var dr = w / h; if (ir > dr) { var dh = h; h = w / ir; y += (dh - h) / 2; } else { var dw = w; w = h * ir; x += (dw - w) / 2; } } else if (effFit.indexOf('-out') != -1) { var iw = img.naturalWidth; if (iw == 0) iw = w; var ih = img.naturalHeight; if (ih == 0) ih = h; var ir = iw / ih; var dr = w / h; if (ir > dr) { var dw = w; w = h * ir; x -= (w - dw) / 2; } else { var dh = h; h = w / ir; y -= (h - dh) / 2; } }
    if (scaleW != 1) { var cx = x + w / 2; w *= scaleW; x = cx - w / 2; }
    if (scaleH != 1) { var cy = y + h / 2; h *= scaleH; y = cy - h / 2; }
    if (moveX != 0) x += moveX; if (moveY != 0) y += moveY;
  } else {
    var ctr = this.CurrTrans; var otr = this.ObjTrans; ctr.ObjTransXY2(this.GetObjTrans(), x, y, x + w, y + h); if (ctr.x1 > ctr.x2) { var tmp = ctr.x1; ctr.x1 = ctr.x2; ctr.x2 = tmp; }
    if (ctr.y1 > ctr.y2) { var tmp = ctr.y1; ctr.y1 = ctr.y2; ctr.y2 = tmp; }
    x = ctr.x1; y = ctr.y1; w = ctr.x2 - x; h = ctr.y2 - y;
  }
  if (w == 0) w = img.naturalWidth; if (sw == 0) sw = img.naturalWidth; if (h == 0) h = img.naturalHeight; if (sh == 0) sh = img.naturalHeight; this.Context2D.drawImage(img, sx, sy, sw, sh, x, y, w, h); return true;
}
JsGraph.prototype.GetTextSize = function (txt, w) {
  var box = this.WorkRect; box.SetPos(0, 0); if (this.TextCanvasRendering) { this.GetCanvasTextSize(txt, box); } else { w = xDefNum(w, 0); this.HtmlTextHandler.GetTextSize(txt, w, box); }
...
JsGraph.prototype.GetTextBox = function (txt, x, y, w) {
  if (!xDef(x)) return this.GetTextBox(txt, 0, 0); if (JsgVect2.Ok(x)) return this.GetTextBox(txt, x[0], x[1], y); var box = this.WorkRect; w = xDefNum(w, 0); var ctr = this.CurrTrans; var cnvsX = ctr.TransX(x); var cnvsY = ctr.TransY(y); if (this.TextCanvasRendering) { this.GetCanvasTextBox(txt, cnvsX, cnvsY, box); } else { this.HtmlTextHandler.GetTextBox(txt, cnvsX, cnvsY, w, box); }
...
JsGraph.prototype.Text = function (txt, x, y, WidthOrMode) {
  if (JsgVect2.Ok(x)) return this.Text(txt, x[0], x[1], y); WidthOrMode = xDefNum(WidthOrMode, 0); var ctr = this.CurrTrans; ctr.ObjTransXY(this.GetObjTrans(), x, y); if (this.GraphClipEnabled && !this.IsPointInsideRect(ctr.x, ctr.y, this.GraphClipOuterXmin, this.GraphClipOuterXmax, this.GraphClipOuterYmin, this.GraphClipOuterYmax)) { return this; }
...
JsGraph.prototype.TextBox = function (txt, x, y, mode, roll, width) {
  if (JsgVect2.Ok(x)) return this.TextBox(txt, x[0], x[1], y, mode, roll); if (this.TextCanvasRendering) { var objTrans = this.SaveTrans(true); this.TransMove(-x, -y); this.TransScale(this.CurrTrans.ScaleX, this.CurrTrans.ScaleY); this.TransRotate(this.TextRotation); this.TransScale(1 / this.CurrTrans.ScaleX, 1 / this.CurrTrans.ScaleY); this.TransMove(x, y); this.AddTrans(objTrans); this.Rect(this.GetTextBox(txt, x, y), mode, roll); this.RestoreTrans(); } else { this.Rect(this.GetTextBox(txt, x, y, width), mode, roll); }
  return this;
}
JsGraph.prototype.DrawCanvasText = function (txt, x, y, mode) {
  this.SetCanvasFont(); var ctr = this.CurrTrans; var otr = this.ObjTrans; var ctx = this.Context2D; var oldFillStyle = ctx.fillStyle; if (mode == 0) { mode = 2; ctx.fillStyle = this.TextColor; }
...
JsGraph.prototype.StartXright = function (x, dx, winBorderLeft) {
  if (x < winBorderLeft) { x = Math.floor(winBorderLeft / dx) * dx; if (x < winBorderLeft) x += dx; }
  return x;
}
JsGraph.prototype.StartXleft = function (x, dx, winBorderRight) {
  if (x > winBorderRight) { x = -Math.floor(-winBorderRight / dx) * dx; if (x > winBorderRight) x -= dx; }
  return x;
}
JsGraph.prototype.StartYup = function (y, dy, winBorderBottom) {
  if (y < winBorderBottom) { y = Math.floor(winBorderBottom / dy) * dy; if (y < winBorderBottom) y += dy; }
  return y;
}
JsGraph.prototype.StartYdown = function (y, dy, winBorderTop) {
  if (y > winBorderTop) { y = -Math.floor(-winBorderTop / dy) * dy; if (y > winBorderTop) y -= dy; }
  return y;
}
...
JsGraph.prototype.GetFrameRect = function () {
  var rect = this.GetTransRect(); if (this.Trans == 'viewport') { rect.w = this.VpInnerWidth; rect.h = this.VpInnerHeight; }
  return rect;
}
...
JsGraph.prototype.AxesX = function (yPos, ArrowSymbol, ArrowSize) {
  yPos = xDefNum(yPos, 0); ArrowSymbol = xDefStr(ArrowSymbol, ''); ArrowSize = xDefNum(ArrowSize, 8); var box = this.GetFrame(); var xMin = box.xmin; var xMax = box.xmax; if (xMin > xMax) { var tmp = xMin; xMin = xMax; xMax = tmp; }
...
JsGraph.prototype.AxesY = function (xPos, ArrowSymbol, ArrowSize) {
  xPos = xDefNum(xPos, 0); ArrowSymbol = xDefStr(ArrowSymbol, ''); ArrowSize = xDefNum(ArrowSize, 8); var box = this.GetFrame(); var xMin = box.xmin; var xMax = box.xmax; if (xMin > xMax) { var tmp = xMin; xMin = xMax; xMax = tmp; }
  var yMin = box.ymin; var yMax = box.ymax; if (yMin > yMax) { var tmp = yMin; yMin = yMax; yMax = tmp; }
...
JsGraph.prototype.TicsX = function (yPos, dx, ticUp, ticDown, skipZero, skipLimit) {
  yPos = xDefNum(yPos, 0); dx = xDefNum(dx, 1); if (dx <= 0) return; ticUp = xDefNum(ticUp, 3); ticDown = xDefNum(ticDown, ticUp); skipZero = xDefBool(skipZero, true); skipLimit = xDefBool(skipLimit, false); if (this.AutoScalePix) { ticUp = this.ScalePix(ticUp, this.ScalePixInt); ticDown = this.ScalePix(ticDown, this.ScalePixInt); }
...
JsGraph.prototype.TicsY = function (xPos, dy, ticRight, ticLeft, skipZero, skipLimit) {
  xPos = xDefNum(xPos, 0); dy = xDefNum(dy, 1); if (dy <= 0) return; ticRight = xDefNum(ticRight, 3); ticLeft = xDefNum(ticLeft, ticRight); skipZero = xDefBool(skipZero, true); skipLimit = xDefBool(skipLimit, false); if (this.AutoScalePix) { ticRight = this.ScalePix(ticRight, this.ScalePixInt); ticLeft = this.ScalePix(ticLeft, this.ScalePixInt); }
...
JsGraph.prototype.AddMarkerDef = function (aName, aMarkerDef) { this.Markers[aName] = aMarkerDef; }
...
JsGraph.prototype.Marker = function (x, y, mode, mat, size) {
  if (JsgPolygon.Ok(x)) return this.Marker(x.X, x.Y, y, mode, x.Size); if (xArray(x) && xArray(y)) {
    size = xDefNum(size, x.length); for (var i = 0; i < size; i++) { this.Marker(x[i], y[i], mode, mat); }
    return this;
  }
  ...
  var drawMode = mode & 3; for (var i = 0; i < symbol.length; i++) {
    var element = symbol[ix]; if (element.type == 'Polygon') {
      var poly = this.WorkPolyMarker.Reset(); var len = element.x.length; for (var j = 0; j < len; j++)poly.AddPoint(element.x[j], element.y[j]); if (JsgMat2.Ok(mat)) { JsgMat2.TransPolyXY(mat, poly.X, poly.Y, poly.Size); }
      this.ScaleAndMovePoly(poly, this.DriverMarkerSize, ctr.x, ctr.y); if (inverse) poly.Invert(); this.Polygon(poly, drawMode + 4);
    }
    else if (element.type == 'Line') {
      var poly = this.WorkPolyMarker.Reset(); poly.AddPoint(element.x1, element.y1); poly.AddPoint(element.x2, element.y2); if (JsgMat2.Ok(mat)) { JsgMat2.TransPolyXY(mat, poly.X, poly.Y, poly.Size); }
...
JsgHtmlTextHandler.AppliedTextStyles = 'color fontFamily fontSize fontStyle fontWeight lineHeight textAlign'.split(' '); JsgHtmlTextHandler.prototype.NewTextStyles = function (from) {
  var styles = {}; var styleNames = JsgHtmlTextHandler.AppliedTextStyles; for (var i = 0; i < styleNames.length; i++) { styles[styleNames[i]] = ''; }
...
JsgHtmlTextHandler.prototype.SameTextStyles = function (styles1, styles2) {
  var styleNames = JsgHtmlTextHandler.AppliedTextStyles; for (var i = 0; i < styleNames.length; i++) { var name = styleNames[i]; if (styles1[name] != styles2[name]) return false; }
  return true;
}
JsgHtmlTextHandler.prototype.Clear = function () {
  for (var i = 0; i < this.Text.length; i++) { this.ClippingDiv.removeChild(this.Text[i]); }
...
JsgHtmlTextHandler.prototype.FindTextSizeInCache = function (s, textClass, styles, aw, box) {
  if (this.CachePtr >= this.Cache.length) return false; aw = xDefNum(aw, -1); var c = this.Cache[this.CachePtr]; if (c.Text == s && c.TextClass == textClass && this.SameTextStyles(c.Styles, styles) && c.ArgWidth == aw) { this.CachePtr++; box.SetSize(c.Width, c.Height); return true; }
...
JsgHtmlTextHandler.prototype.CreateTextNode = function (s, w) {
  var txt = document.createElement('div'); this.CopyTextStyles(this.TextStyles, txt.style)
...
JsgHtmlTextHandler.prototype.HandleText = function (mode, s, x, y, w, box) {
  this.GetTextSize(s, w, box); box.w += 2 * this.TextHPad; box.h += 2 * this.TextVPad; var top = y; var left = x; var padleft = this.TextHPad; var padright = this.TextHPad; if (this.TextHAlign == 'center') left -= box.w / 2; if (this.TextHAlign == 'right') left -= box.w; if (this.TextVAlign == 'middle') top -= box.h / 2; if (this.TextVAlign == 'bottom') top -= box.h; if (w == 0) {
    var cw = this.ClippingDiv.offsetWidth; var right = left + box.w; var newleft = left; var newright = right; var borderCrossed = false; if (left < 0 && right > 0) { padleft = this.TextHPad + left; if (padleft < 0) padleft = 0; newleft = 0; borderCrossed = true; }
  ...
  box.SetPos(left, top); if (mode == 1) { var txtNode = this.CreateTextNode(s, w); txtNode.style.left = left + padleft + 'px'; txtNode.style.top = top + this.TextVPad + 'px'; this.ClippingDiv.appendChild(txtNode); this.Text.push(txtNode); }
}
Object.assign(globalThis, {
  JsgColor, JsgVect2, JsgMat2, JsgRect, JsgGradient, JsgPolygon, JsgPolygonList,
  JsgSnapshot, JsgTrans, JsgTrans2D, JsgAttrs, NewGraph2D, JsGraph, JsgHtmlTextHandler,
});
export {
  JsgColor, JsgVect2, JsgMat2, JsgRect, JsgGradient, JsgPolygon, JsgPolygonList,
  JsgSnapshot, JsgTrans, JsgTrans2D, JsgAttrs, NewGraph2D, JsGraph, JsgHtmlTextHandler,
