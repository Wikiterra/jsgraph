...
var xOnLoadFinished = false; var xEventManager = {
    DomReadyHandlers: new xCallbackChain(), MyDomReadyHandlers: [], DomReadyFired: false, PageLoadHandlers: new xCallbackChain(), MyPageLoadHandler: null, PageLoadFired: false, PageUnloadHandlers: new xCallbackChain(), OldWindowOnUnloadHandler: null, MyPageUnloadHandler: null, LayoutChangeHandlers: new xCallbackChain(), MyLayoutChangeHandler: null, WindowResizeHandlers: new xCallbackChain(), WindowResizeTimer: null, MyWindowResizeHandler: null, DisplayChangeHandlers: new xCallbackChain(), AddDomReadyHandler: function (aFunc) {
        ...
        var self = this; this.LayoutChangeTimer = setTimeout(function OnLayoutChangeEvent(aArg) { clearTimeout(self.LayoutChangeTimer); self.LayoutChangeTimer = null; xOptions.Transform.OffsetElement = null; self.LayoutChangeHandlers.Call(aArg); }, this.LayoutChangeTimerDelay);
...
var xOptions = { Transform: { PropertyNames: ['webkitTransform', 'MozTransform', 'msTransform', 'OTransform', 'transform'], PropertyName: '?', OffsetElement: null, OffsetX: 0, OffsetY: 0 } }; function xGetTransformPropertyName(e) {
    var tf = xOptions.Transform; if (tf.PropertyName != '?') return tf.PropertyName; var names = tf.PropertyNames; var len = names.length; for (var i = 0; i < len; i++) { var name = names[i]; if (xDef(e.style[name])) { tf.PropertyName = name; return name; } }
...
function xGetTransformDocOffset(e) {
    var tf = xOptions.Transform; if (tf.OffsetElement != e) { tf.OffsetElement = e; tf.OffsetX = 0; tf.OffsetY = 0; if (!xTransform(e, 'translate(0px,0px)')) return tf; tf.OffsetX = xPageX(e); tf.OffsetY = xPageY(e); }
    return tf;
}
function xTransformNone(e) { e = xElement(e); if (!xTransform(e, 'none')) xMoveTo(e, 0, 0); }
function xTransformTranslate(e, x, y, useWinOrigin) {
    e = xElement(e); useWinOrigin = xDefBool(useWinOrigin, false); var tx = x, ty = y; if (useWinOrigin) { var tf = xGetTransformDocOffset(e); tx -= tf.OffsetX; ty -= tf.OffsetY; }
...
function xTransformTranslateScale(e, x, y, w, h, wRef, hRef, useWinOrigin) {
    e = xElement(e); useWinOrigin = xDefBool(useWinOrigin, false); var tx = x, ty = y; if (useWinOrigin) { var t = xGetTransformDocOffset(e); tx -= t.OffsetX; ty -= t.OffsetY; }
...
var xClipboardBuffer = null; function xToClipboard(text) {
    if (xDef(window.clipboardData)) { window.clipboardData.setData('Text', text); return true; }
    else if (document.queryCommandSupported && document.queryCommandSupported('copy')) {
        var textArea = document.createElement("textarea"); textArea.style.position = 'fixed'; textArea.style.width = '2em'; textArea.style.height = '2em'; textArea.style.padding = 0; textArea.style.border = 'none'; textArea.style.outline = 'none'; textArea.style.boxShadow = 'none'; textArea.style.background = 'transparent'; textArea.value = text; document.body.appendChild(textArea); textArea.select(); var done = false; try { done = document.execCommand('copy'); } catch (clipboardError) { }
...
xMultiImage.prototype.Show = function (aImageNumber) { xChangeImage(this.ImgID, this.Images[aImageNumber]); }; var xDbgMess = ''; var xDbgSep = '\n'; function xDbg(aMess) {
    if (aMess) { xDbgMess += aMess + xDbgSep; }
    else { alert(xDbgMess); }
}
...
CZoom.prototype.Init = function (aImgUrlList) { this.ImgUrlList = aImgUrlList; }; CZoom.prototype.ForceInit = function () {
    if (this.InitExecuted) { return; }
    this.CreateHtmlObjects(); this.InitExecuted = true; this.InitForced = true;
}; CZoom.prototype.FindAndPreloadImages = function () {
    var urlList = []; var callList = this.FindZoomFunctionCalls(); for (var i = 0; i < callList.length; i++) { urlList.push(this.GetPathFromZoomFunctionCall(callList[i])); }
    if (urlList.length > 0) { this.Preload(urlList); }
}
CZoom.prototype.FindZoomFunctionCalls = function () {
    var callList = []; var el = xGetByTag('*'); for (var i = 0; i < el.length; i++) {
        var e = el[i]; if (e.onclick) { var src = this.IsZoomFunctionCall(e.onclick); if (src != '') { callList.push(src); } }
...
CZoom.prototype.GetPathFromZoomFunctionCall = function (aCallStr) {
    var p = aCallStr.indexOf(this.ZoomFunctionName); if (p < 0) { return ''; }
...
CZoom.prototype.LoadPicsOnPageLoad = function (aImgUrlList) {
    var me = this; function AfterPageLoad() { if (aImgUrlList.length > 0) { me.Preload(aImgUrlList); } }
    xOnLoad(AfterPageLoad);
}; CZoom.prototype.Preload = function (aImgUrlList) {
    if (aImgUrlList.length == 2 && xArray(aImgUrlList[1])) { IC.PreloadImages(aImgUrlList[1], aImgUrlList[0]); }
    else { IC.PreloadImages(aImgUrlList); }
    this.PreloadExecuted = true;
}; CZoom.prototype.Diagnose = function () {
    var ics = IC.GetStatus(); var s = ''; if (this.ErrMsg != '') { s += 'Errors:\n'; s += this.ErrMsg + '\n'; }
...
}; CZoom.prototype.AddError = function (aMsg) { this.ErrMsg += aMsg + '\n'; }; CZoom.prototype.CreateHtmlObjects = function () {
...
}; CZoom.prototype.ReZoom = function () { this.PrevPosX = this.BigPosX; this.PrevPosY = this.BigPosY; this.PrevWidth = this.BigWidth; this.PrevHeight = this.BigHeight; this.IsReZooming = true; this.ComputeZoomParametersAndTriggerZoom(); }
CZoom.prototype.ZoomIn = function (aImgName, aBigImgUrl, aXOffset, aYOffset, aRelEleID) {
    this.ResetTimer(); if (!this.InitExecuted) {
        if (this.EnableInitOnClick) { this.AddError('ZoomIn: Zoom not initialized - forcing init now!\nCheck Zoom.Init and ensure no onload is in body tag!'); this.ForceInit(); if (!this.HtmlWritten) { this.AddError('ZoomIn: forced Init failed, give up here.'); return; } } else {
            if (this.DebugOn) { this.AddError('ZoomIn: Zoom not initialized!\nCheck Zoom.Init and ensure no onload is in body tag\nor set Zoom.EnableInitOnClick = true;'); this.Diagnose(); }
            return;
        }
    }
    ...
    if ((this.State != this.CState.Hidden) && (bigImgID != -1) && (bigImgID == this.BigImgID)) {
        if (this.State == this.CState.Loading) { this.State = this.CState.Hidden; return; }
...
CZoom.prototype.HideZoomImg = function () { if (this.HideSmall) xShow(this.SmallImg); xHide(this.ZoomImg); xTransformTranslateScale(this.ZoomImg, 0, 0, 10, 10, this.BigNativeWidth, this.BigNativeHeight); this.ZoomImg.src = ''; this.State = this.CState.Hidden; }; CZoom.prototype.Enable = function () { this.Enabled = true; }; CZoom.prototype.Disable = function () { this.Enabled = false; }; CZoom.prototype.GetSmallImgData = function () { this.SmallWidth = xWidth(this.SmallImg) + 2 * this.BorderWidth; this.SmallHeight = xHeight(this.SmallImg) + 2 * this.BorderWidth; this.SmallPosX = xPageX(this.SmallImg) + (xWidth(this.SmallImg) - this.SmallWidth) / 2 + this.AddPosX; this.SmallPosY = xPageY(this.SmallImg) + (xHeight(this.SmallImg) - this.SmallHeight) / 2 + this.AddPosY; }; CZoom.prototype.OnLoad = function (aImgID) {
    this.ResetTimer(); if ((this.State == this.CState.Loading) && (this.BigImgID == aImgID)) {
        var imgState = IC.Image(aImgID).CacheState; if (imgState == IC.CState.Loaded) { this.StartZoom(); }
        else if (imgState == IC.CState.Error || imgState == IC.CState.Abort) { this.State = this.CState.Hidden; this.ShowErrorMessage(); }
    }
}; CZoom.prototype.Range = function (aValue, aMin, aMax) { return aMin + (aMax - aMin) * aValue; }; CZoom.prototype.StartZoom = function () {
    var me = this; function go() { me.ResetTimer(); me.HideMessages(); me.ZoomImg.onload = null; me.ComputeZoomParametersAndTriggerZoom(); }
    function TryAgain() {
        this.ResetTimer(); if (me.ZoomImg.complete) { go(); }
    ...
    this.StartTime = xTimeMS(); this.SetTimer(TryAgain, 100);
    ...
    if (this.CurrHAlign == 'Left') { this.BigPosX = clX + this.HMargin; } else if (this.CurrHAlign == 'Right') { this.BigPosX = (clX + clW) - this.BigWidth - this.HMargin; } else if (this.CurrHAlign == 'Relative') {
        var ref = this.SmallImg; if (this.CurrTargetEle) { ref = this.CurrTargetEle; }
        this.BigPosX = xPageX(ref) + this.CurrTargetOffX;
    } else {
        var dxCenter = 1; if (this.BigWidth <= clW) { dxCenter = (this.BigWidth - this.SmallWidth) / (clW - this.SmallWidth); if (dxCenter < 0) { dxCenter = 0; } }
    ...
    if (this.CurrVAlign == 'Top') { this.BigPosY = clY + this.VMargin; } else if (this.CurrVAlign == 'Bottom') { this.BigPosY = (clY + clH) - this.BigHeight - this.VMargin; } else if (this.CurrVAlign == 'Relative') {
        var ref = this.SmallImg; if (this.CurrTargetEle) { ref = this.CurrTargetEle; }
        this.BigPosY = xPageY(ref) + this.CurrTargetOffY;
    } else {
        var dyCenter = 1; if (this.BigHeight <= clH) { dyCenter = (this.BigHeight - this.SmallHeight) / (clH - this.SmallHeight); if (dyCenter < 0) { dyCenter = 0; } }
    ...
    this.ComputeTimeSpan(); this.StartTime = xTimeMS(); var me = this; this.RequestAnimationFrame(function () { me.Enlarge(); });
...
CZoom.prototype.Enlarge = function () {
    ...
    var x = this.Range(param, srcX, this.BigPosX); var y = this.Range(param, srcY, this.BigPosY); var w = this.Range(param, srcW, this.BigWidth); var h = this.Range(param, srcH, this.BigHeight); xTransformTranslateScale(this.ZoomImg, x, y, w, h, this.BigNativeWidth, this.BigNativeHeight, true); if (this.State != this.CState.ZoomIn) {
        this.HideMessages(); xShow(this.ZoomImg); if (this.HideSmall) { xHide(this.SmallImg); }
        this.State = this.CState.ZoomIn;
    }
    var me = this; if (eom) { this.State = this.CState.Zoomed; this.IsReZooming = false; this.SetTimer(function () { me.MonitorOutOfWindowOrResize(); }, 200); }
    else { this.RequestAnimationFrame(function () { me.Enlarge(); }); }
...
CZoom.prototype.MonitorOutOfWindowOrResize = function () {
    ...
    this.ComputeTimeSpan(); this.State = this.CState.ZoomOut; var me = this; this.RequestAnimationFrame(function () { me.Shrink(); });
...
CProgressbar.prototype.Start = function (aAction, aTotal) {
    this.SP++; if (this.SP > this.MaxSP) { this.MaxSP = this.SP; }
...
CProgressbar.prototype.End = function (bAll) {
    if (this.SP < 0) { this.SP = -1; return; }
    this.ActionStack[this.SP] = ''; this.TextStack[this.SP] = ''; this.NStack[this.SP] = 0; this.OfStack[this.SP] = 0; this.CounterStack[this.SP] = 0; this.SP--; if (this.SP < 0 || bAll) {
        this.Hide(); if (bAll) { this.SP = -1; }
        return;
    }
}
...
CProgressbar.prototype.MakeMsg = function () {
    if (this.SP < 0) return; var s = ''; for (var i = 0; i <= this.MaxSP; i++) { s += this.MakeLine(i); }
    return s;
}
CProgressbar.prototype.MakeLine = function (i) {
    var line = '<div>'; var action = this.ActionStack[i]; if (action != '') {
        if (i == 0) { line += '<b>'; }
...
CProgressbar.prototype.MakeBar = function (i) {
    var prog = 0; if (this.NStack[i] > 0) { if (this.OfStack[i] == 0) { prog = this.NStack[i] / (this.NStack[i] + 2); } else { prog = this.NStack[i] / this.OfStack[i]; } } else { prog = this.CounterStack[i] / (this.CounterStack[i] + 2); }
...
var WIKI_MinHiliWordLength = 1; var WIKI_MaxHiliItems = 100; var WIKI_NumHiliColors = 3; var WIKI_KeyPressed = false; var WIKI_HtmlBody = null; var WIKI_RedirectList = []; var WIKI_HiliStateOn = false; var WIKI_NumHiliItems = 0; function EditPage(event, aHeaderText) {
    if (PAGE_MODE == '') {
        if (ENABLE_EDIT) {
            var asp = ASP_PAGE; var op = '&op=edit'; if (!IS_LOGED_IN && !event.shiftKey) { op = '&op=login'; }
            if (aHeaderText) {
                if (op) { op += '&'; }
                op += 'hd=' + UrlEncode(aHeaderText);
            }
            location.href = asp + '?page=' + UrlEncode(PAGE_NAME) + op; event.PreventDefault(); event.StopPropagation();
        }
    } else { }
}
...
function OnDocKeyDown(ev) {
    if (!WIKI_KeyPressed && (ev.keyCode == 13 || (ev.keyCode >= 49 && ev.keyCode <= 57))) {
        var i = ev.keyCode - 49; if (i < 0) { i = 0; }
...
function OnDblCklick(event) {
    var target = event.target; while (target && target.nodeName.charAt(0) != 'H') { target = target.parentNode; }
    if (!target) { return; }
...
function InitWikiJS() {
    WIKI_HtmlBody = xGet('body'); var oHtmlList = xGetByTag("html"); if (oHtmlList.length > 0) { xAddEvent(oHtmlList[0], 'keydown', OnDocKeyDown, false); }
...
function LayoutMaximize(removeLimitWidth) {
    if (xFunc(window.OnLayoutMaximize)) { try { OnLayoutMaximize(); } catch (e) { } }
...
function LayoutNormal() {
    if (xFunc(window.OnLayoutNormal)) { try { OnLayoutNormal(); } catch (e) { } }
...
function LayoutFullscreenOn() {
    if (xFunc(window.OnFullscreenOn)) { try { OnFullscreenOn(); } catch (e) { } }
...
function LayoutFullscreenOff() {
    if (xFunc(window.OnFullscreenOff)) { try { OnFullscreenOff(); } catch (e) { } }
...
function AddToCookie(aName, aValue, aSep, aDays) {
...
function SplitWords(words) {
    ...
    var pl = words.split('"'); if (pl.length > 1) {
        for (var i = 1; i < pl.length; i += 2) { pl[i] = pl[i].replace(/ /g, '%20'); pl[i] = pl[i].replace(/\|/g, '%7C'); }
        words = ''; for (var i = 0; i < pl.length; i++) {
            if (i > 0) { words += '"'; }
            words += pl[i];
        }
    }
    words = words.replace(/\|/g, ' '); pl = words.split(' '); for (var i = 0; i < pl.length; i++) {
        var s = pl[i]; s = s.replace(/%20/g, ' '); s = s.replace(/%7C/g, '|'); if (s.length > 2 && s.charAt(0) == '"' && s.charAt(s.length - 1) == '"') { s = s.substr(1, s.length - 2); }
        pl[i] = s;
    }
    return pl;
}
...
function highlightWord(node, word, n) {
    if (word.length < WIKI_MinHiliWordLength || WIKI_NumHiliItems > WIKI_MaxHiliItems) { return false; }
    ...
    if (node.nodeType == 3) {
        var tempNodeVal = node.nodeValue.toLowerCase(); if (tempNodeVal.indexOf(word) != -1) {
            var pn = xParent(node, true); if (pn.className.substr(0, 10) != 'searchword') {
                var nv = node.nodeValue; var ni = tempNodeVal.indexOf(word); var before = xCreateTextNode(nv.substr(0, ni)); if (!before) { return false; }
...
function highlightRegExp(node, word, n) {
    if (WIKI_NumHiliItems > WIKI_MaxHiliItems) { return false; }
    ...
    if (node.nodeType == 3) {
        try {
            var re = new RegExp(word, 'ig'); var rematch = re.exec(node.nodeValue); if (rematch && xDef(RegExp) && xDef(RegExp.leftContext) && xDef(RegExp.rightContext) && xDef(RegExp.lastMatch) && (RegExp.lastMatch != '')) {
                var pn = xParent(node, true); if (xStr(pn.className) && pn.className.substr(0, 10) != 'searchword') {
                    var before = xCreateTextNode(RegExp.leftContext); if (!before) { return false; }
...
function MarkSearch(htmlRange) {
    htmlRange = xDefStr(htmlRange, 'Wiki'); if (WIKI_HiliWordList.length == 0) { return; }
...
function DoMarkSearch(aHtmlObj, aSearchList) {
    var n = aSearchList.length / 2; var c = 0; for (var i = 0; i < aSearchList.length; i += 2) {
        var pattern = aSearchList[i + 1]; if (aSearchList[i] == 1) { highlightRegExp(aHtmlObj, pattern, c); } else { highlightWord(aHtmlObj, decodeHtml(pattern).toLowerCase(), c); }
        c++;
    }
}
function ToggleMarks() {
    var tags = xGetByTag('span'); if (tags.length > 0 && xDef(tags[0].className)) {
        if (WIKI_HiliStateOn) { for (var i = 0; i < tags.length; i++) { if (xDef(tags[i].className) && tags[i].className.substr(0, 10) == 'searchword') { tags[i].className = '_' + tags[i].className; } } } else { for (var i = 0; i < tags.length; i++) { if (xDef(tags[i].className) && tags[i].className.substr(0, 11) == '_searchword') { tags[i].className = tags[i].className.substr(1, 11); } } }
        WIKI_HiliStateOn = !WIKI_HiliStateOn;
    }
}
function MarkupMathText(aText, aBlockMode) {
    if (MATH_PROCESSOR == 'jsMath') { if (aBlockMode) { return '<div class="math">' + aText + '</div>'; } else { return '<span class="math">' + aText + '</span'; } }
...
function ProcessMathText(HtmlElement) {
    if (MATH_PROCESSOR == 'jsMath') { jsMath.ProcessBeforeShowing(HtmlElement); }
    if (MATH_PROCESSOR == 'MathJax') { MathJax.Hub.Queue(["Typeset", MathJax.Hub, HtmlElement]); }
}
var WikiMenuBarHandling = {
    menuIsOpen: false, docClickInstalled: false, bodyCursor: '', GetChildUl: function (ele) {
        var childList = ele.childNodes; for (var i = 0; i < childList.length; i++) {
            var child = childList[i]; if (child.tagName == 'UL') { return child; }
            var subChild = this.GetChildUl(child); if (subChild) { return subChild; }
        }
        return null;
    }, GetChildLiList: function (ul) {
        var liList = []; var ulChildList = ul.childNodes; for (var i = 0; i < ulChildList.length; i++) { var ulChild = ulChildList[i]; if (ulChild.tagName == 'LI') { liList.push(ulChild); } }
        return liList;
    }, CloseSubmenuTree: function (li) { var ul = this.GetChildUl(li); if (!ul) return; xRemoveClass(li, 'is-open'); ul.style.display = 'none'; var liList = this.GetChildLiList(ul); for (var i = 0; i < liList.length; i++) { this.CloseSubmenuTree(liList[i]); } }, CloseOtherSubmenus: function (currentLi) { var parentUl = currentLi.parentNode; var liList = this.GetChildLiList(parentUl); for (var i = 0; i < liList.length; i++) { var li = liList[i]; if (li != currentLi) { this.CloseSubmenuTree(li); } } }, OnDocumentClick: function () { this.CloseAllMenus(); document.body.style.cursor = this.bodyCursor; this.menuIsOpen = false; }, OnClick: function (event) {
        var li = event.target; while (li && li.tagName != 'LI') { li = li.parentNode; }
        if (!li) return; var childUl = this.GetChildUl(li); if (!childUl) return; xAddClass(li, 'is-open'); if (childUl.style.display == 'none') {
            childUl.style.display = 'block'; this.CloseOtherSubmenus(li); event.PreventDefault(); event.StopPropagation(); if (!this.menuIsOpen) { document.body.style.cursor = 'pointer'; }
    ...
    }, RegisterSubmenuClickHandlers: function (ul) { ul.style.display = 'none'; var childLiList = this.GetChildLiList(ul); for (var i = 0; i < childLiList.length; i++) { var li = childLiList[i]; var childUl = this.GetChildUl(li); if (childUl) { var me = this; xAddEvent(li, 'click', function (event) { me.OnClick(event); }); this.RegisterSubmenuClickHandlers(childUl); } } }, RegisterMenuClickHandlers: function (menuDiv) { var menuUl = this.GetChildUl(menuDiv); if (!menuUl) return; this.RegisterSubmenuClickHandlers(menuUl); menuUl.style.display = ''; }, Init: function () {
        var menuDivList = xGetAll('div.menubar'); for (i = 0; i < menuDivList.length; i++) { this.RegisterMenuClickHandlers(menuDivList[i]); }
        this.bodyCursor = body.style.cursor;
    }, CloseAllMenus: function () { var menuList = xGetAll('div.menubar'); for (i = 0; i < menuList.length; i++) { var menuDiv = menuList[i]; var ul = this.GetChildUl(menuDiv); if (ul) { var liList = this.GetChildLiList(ul); for (var j = 0; j < liList.length; j++) { this.CloseSubmenuTree(liList[j]); } } } },
}; var UrlParams = {
    params: null, Parse: function (url) {
        if (!xStr(url) && this.params) return; this.params = {}; url = xDefStr(url, location.href); var pos = url.indexOf('?'); if (pos < 0) return; var paramListStr = url.substr(pos + 1); if (paramListStr == '') return; var pos = paramListStr.indexOf('#'); if (pos >= 0) paramListStr = paramListStr.substr(0, pos); if (paramListStr == '') return; var paramList = paramListStr.split('&'); for (i = 0; i < paramList.length; i++) {
    ...
    }, GetStr: function (name, def) { this.Parse(); name = name.toLowerCase(); if (xDef(this.params[name])) return this.params[name]; return xDef(def) ? def : ''; }, GetInt: function (name, def) {
        this.Parse(); var param = this.GetStr(name); if (param != '') { var val = parseInt(param); if (!isNaN(val)) return val; }
        return xDef(def) ? def : '';
    }, GetNum: function (name, def) {
        this.Parse(); var param = this.GetStr(name); if (param != '') { var val = parseFloat(param); if (!isNaN(val)) return val; }
        return xDef(def) ? def : '';
    },
}; var OnOffSections = {
    HandleUrlParameters: function () {
        var idStr = UrlParams.GetStr('open'); if (idStr != '') { this.SetStates(idStr, true); }
        var idStr = UrlParams.GetStr('close'); if (idStr != '') { this.SetStates(idStr, false); }
        var url = location.href; var p = url.indexOf('#'); if (p >= 0) { var urlHeader = url.substr(p + 1); if (urlHeader != '') { urlHeader = urlHeader.replace(/H_/, '').replace(/_/g, ' '); var onOffHeaderList = xGetAll('[data-onoffid]'); for (var i = 0; i < onOffHeaderList.length; i++) { var headerElement = onOffHeaderList[i]; var headerText = xInnerHTML(headerElement); if (headerText.indexOf(urlHeader) == 0) { var id = headerElement.dataset.onoffid; this.SetState(id, true); break; } } } }
    }, SetStates: function (idStr, state) { var idList = idStr.split(','); for (var i = 0; i < idList.length; i++) { var id = idList[i]; if (id != '') { this.SetState(id, state); } } }, SetState: function (id, state) { var grp = 'SEL_GRP_' + id; var stateStr = state ? 'ON' : 'OFF'; if (xDef(window[grp])) { try { SEL(window[grp], [id + stateStr], true); } catch (e) { } } },
}; xOnDomReady(function () { WikiMenuBarHandling.Init(); }); xOnLoad(function () { UrlParams.Parse(); OnOffSections.HandleUrlParameters(); MarkSearch(); });
Object.assign(globalThis, { xClass2Type, xType, xDef, xAny, xObj, xObjOrNull, xFunc, xFuncOrNull, xArray, xStr, xNum, xBool, xIsNumeric, xDefAny, xDefAnyOrNull, xDefObj, xDefObjOrNull, xDefFunc, xDefFuncOrNull, xDefArray, xDefStr, xDefNum, xDefBool, xArgsToArray, xFStr, xArrFind, xArrFindIndex, xArrForEach, xArrayMap, xArrRemove, xArrRemoveAll, xGet, xGetFirst, xGetAll, xElement, xDataset, xInnerHTML, xInnerText, xTagName, xShow, xHide, xVisibility, xDisplay, xIsDisplayed, xMoveTo, xLeft, xTop, xOpacity, xResizeTo, xElementWidth, xWidth, xScrollWidth, xNaturalWidth, xElementHeight, xHeight, xScrollHeight, xNaturalHeight, xGetCS, xSetCW, xSetCH, xClientWidth, xClientHeight, xPageX, xPageY, xIsRoot, xIsElementAndNotRoot, xScrollLeft, xScrollTop, xZIndex, xCursor, xStyle, xMaskRegExp, xHasClass, xAddClass, xRemoveClass, xToggleClass, xSetClassIf, xSetEnabled, xSetDisabled, xParent, xCreateElement, xCreateTextNode, xHasChildNodes, xChildNodes, xAppendChild, xInsertBefore, xRemoveChild, xGetByTag, xGetByClass, xAddEvent, xRemoveEvent, xEvent, xImgOnLoad, xCallbackChain, xOnLoadFinished, xEventManager, xAddEventLayoutChange, xRemoveEventLayoutChange, xTriggerEventLayoutChange, xAddEventDisplayChange, xRemoveEventDisplayChange, xTriggerEventDisplayChange, xAddEventWindowResize, xRemoveEventWindowResize, xOnDomReady, xOnLoad, xOnUnload, xOptions, xGetTransformPropertyName, xSupportsTransform, xTransform, xTransformOrigin, xGetTransformDocOffset, xTransformNone, xTransformTranslate, xTransformTranslateScale, xClipboardBuffer, xToClipboard, xTimeMS, xImage, xChangeImage, xMultiImage, xDbgMess, xDbgSep, xDbg, xDbgOut, xDbgApp, xSetCookie, xGetCookie, xDeleteCookie, htmlString, xDebug, xDebugOutId, xLog, xClearLog, CImgCache, IC, Zoom, ZoomInit, ZoomPics, ZoomDebug, ZoomIn, ZoomOut, ZoomEnable, ZoomDisable, CZoom, CProgressbar, Progressbar, WIKI_MinHiliWordLength, WIKI_MaxHiliItems, WIKI_NumHiliColors, WIKI_KeyPressed, WIKI_HtmlBody, WIKI_RedirectList, WIKI_HiliStateOn, WIKI_NumHiliItems, EditPage, ShowUploadForm, ShowWikiFunctions, OnDocKeyDown, UrlEncode, Trim, OnDblCklick, InitWikiJS, LayoutMaximize, LayoutNormal, IsLayoutMaximized, LayoutFullscreenOn, LayoutFullscreenOff, IsLayoutFullscreen, AddToCookie, AddCBReq, SEL, SplitWords, decodeHtml, highlightWord, highlightRegExp, MarkSearch, DoMarkSearch, ToggleMarks, MarkupMathText, ProcessMathText, WikiMenuBarHandling, UrlParams, OnOffSections });
